...
//     } else if (activeTab === 3) {
//       const newErrors = {};
//       const requiredFields = ['salutation','name','address','mobile1','aadhar_number','pan_no','dob','occupation','taluka','district','pincode','nomineeName','nomineeRelation','nomineeAge'];
//       requiredFields.forEach((field) => {
//         if (!formData[field]) newErrors[field] = 'This field is required';
//       });
...
function BookingForm() {
  ...
  const [formData, setFormData] = useState({
    ...
    nominiSalutation: '',
    email: '',
    nominee_first_name: '', // NEW: Nominee First Name
    nominee_middle_name: '', // NEW: Nominee Middle Name
    nominee_last_name: '', // NEW: Nominee Last Name
    nomineeRelation: '',
    nomineeAge: '',
    type: 'cash',
    financer_id: '',
    ...
    try {
      ...
      if (response.data.success && response.data.data.customers.length > 0) {
        ...
        setFormData((prev) => ({
          ...prev,
          salutation: customer.bookings?.[0]?.customerDetails?.salutation || '',
          first_name: customer.bookings?.[0]?.customerDetails?.first_name || customer.first_name || '',
          middle_name: customer.bookings?.[0]?.customerDetails?.middle_name || customer.middle_name || '',
          last_name: customer.bookings?.[0]?.customerDetails?.last_name || customer.last_name || '',
          pan_no: customer.pan || '',
          address: customer.address || '',
          taluka: customer.taluka || '',
          district: customer.district || '',
          ...
          dob: formattedDob,
          occupation: customer.bookings?.[0]?.customerDetails?.occupation || '',
          nominiSalutation: customer.bookings?.[0]?.customerDetails?.nominiSalutation || '',
          email: customer.bookings?.[0]?.customerDetails?.email || ''
        }));
      } else {
        setSearchError('No customer found with this PAN/Aadhar');
      }
  ...
  try {
    ...
    const formDataToSet = {
      ...
      first_name: bookingData.customerDetails?.firstName || '',
      middle_name: bookingData.customerDetails?.middleName || '',
      last_name: bookingData.customerDetails?.lastName || '',
      pan_no: bookingData.customerDetails?.panNo || '',
      dob: bookingData.customerDetails?.dob?.split('T')[0] || '',
      occupation: bookingData.customerDetails?.occupation || '',
      address: bookingData.customerDetails?.address || '',
      ...
      // Updated nominee field mappings
      nominee_first_name: bookingData.customerDetails?.nomineeFirstName || '',
      nominee_middle_name: bookingData.customerDetails?.nomineeMiddleName || '',
      nominee_last_name: bookingData.customerDetails?.nomineeLastName || '',
      nomineeRelation: bookingData.customerDetails?.nomineeRelation || '',
      nomineeAge: bookingData.customerDetails?.nomineeAge || '',
      type: bookingData.payment?.type?.toLowerCase() || 'cash',
      financer_id: bookingData.payment?.financer?._id || '',
  ...
  const handleNextTab = () => {
    if (activeTab === 1) {
      ...
      const newErrors = {};
      const requiredFields = ['salutation', 'first_name', 'last_name', 'address', 'mobile1', 'aadhar_number', 'pan_no', 'dob', 'occupation', 'taluka', 'district', 'pincode', 'nominiSalutation', 'email', 'nominee_first_name', 'nominee_last_name', 'nomineeRelation', 'nomineeAge'];
      requiredFields.forEach((field) => {
        if (!formData[field]) newErrors[field] = 'This field is required';
      });
      if (formData.mobile1 && !validateMobileNumber(formData.mobile1)) newErrors.mobile1 = 'Invalid mobile number';
  ...
  const handleSubmit = async (e) => {
    ...
    const requiredFields = ['verticle_id', 'model_id', 'model_color', 'branch', 'customer_type', 'first_name', 'last_name', 'address', 'mobile1', 'aadhar_number', 'pan_no'];
    let formErrors = {};
    requiredFields.forEach((field) => {
      if (!formData[field]) formErrors[field] = 'This field is required';
    });
              ...
              <>
                ...
                <div className="user-details">
                  ...
                  <div className="input-box">
                    ...
                    <CInputGroup>
                      <CInputGroupText className="input-icon"><CIcon icon={cilUser} /></CInputGroupText>
                      <CFormInput name="first_name" value={formData.first_name} onChange={handleChange} />
                    </CInputGroup>
                  ...
                  <div className="input-box">
                    ...
                    <CInputGroup>
                      <CInputGroupText className="input-icon"><CIcon icon={cilUser} /></CInputGroupText>
                      <CFormInput name="middle_name" value={formData.middle_name} onChange={handleChange} />
                    </CInputGroup>
                  </div>
                  ...
                  <div className="input-box">
                    ...
                    <CInputGroup>
                      <CInputGroupText className="input-icon"><CIcon icon={cilUser} /></CInputGroupText>
                      <CFormInput name="last_name" value={formData.last_name} onChange={handleChange} />
                    </CInputGroup>
                  ...
                  <div className="input-box">
                    <div className="details-container">
                      <span className="details">Nominee First Name</span>
                      <span className="required">*</span>
                    </div>
                    <CInputGroup>
                      <CInputGroupText className="input-icon"><CIcon icon={cilUser} /></CInputGroupText>
                      <CFormInput name="nominee_first_name" value={formData.nominee_first_name} onChange={handleChange} />
                    </CInputGroup>
                    {errors.nominee_first_name && <p className="error">{errors.nominee_first_name}</p>}
                  </div>

                  {/* Nominee Middle Name (Optional) */}
                  <div className="input-box">
                    <div className="details-container">
                      <span className="details">Nominee Middle Name</span>
                    </div>
                    <CInputGroup>
                      <CInputGroupText className="input-icon"><CIcon icon={cilUser} /></CInputGroupText>
                      <CFormInput name="nominee_middle_name" value={formData.nominee_middle_name} onChange={handleChange} />
                    </CInputGroup>
                  </div>
                  ...
                  <div className="input-box">
                    <div className="details-container">
                      <span className="details">Nominee Last Name</span>
                      <span className="required">*</span>
                    </div>
                    <CInputGroup>
                      <CInputGroupText className="input-icon"><CIcon icon={cilUser} /></CInputGroupText>
                      <CFormInput name="nominee_last_name" value={formData.nominee_last_name} onChange={handleChange} />
                    </CInputGroup>
                    {errors.nominee_last_name && <p className="error">{errors.nominee_last_name}</p>}
                  </div>
                  ...
                  <div className="input-box">
                    ...
                    <CInputGroup>
                      <CInputGroupText className="input-icon"><CIcon icon={cilBirthdayCake} /></CInputGroupText>
                      <CFormInput type="number" name="nomineeAge" value={formData.nomineeAge} onChange={handleChange} />
                    </CInputGroup>
                    {errors.nomineeAge && <p className="error">{errors.nomineeAge}</p>}
                  </div>
                </div>
