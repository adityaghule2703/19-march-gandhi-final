...
//     // Submit payment
//     const response = await axiosInstance.post('/ledger/receipt', paymentData);
//     const receipt = response.data.data?.receipt || response.data.data;
...
//           r._id === newReceiptId || r.id === newReceiptId
//         );
        
//         console.log('New receipt index:', newReceiptIndex);
        
//         if (newReceiptIndex === -1) {
//           console.warn('New receipt not found in list, trying alternative search...');
//           // Try to find by receipt number
//           const altIndex = allReceipts.findIndex(r => 
//             r.receiptNumber === receipt.receiptNumber
//           );
//           if (altIndex !== -1) {
//             console.log('Found by receipt number at index:', altIndex);
//           }
//         }
        
//         // Determine if this is the FIRST receipt EVER for this booking
//         // (not just index 0, but actual first in chronological order)
...
//           // IMPORTANT: We need to pass whether this is the FIRST receipt
//           // The print function needs to know this to decide which template to use
//           window.printReceiptCallback(newReceiptId, formData.bookingId, isFirstReceiptEver ? 0 : 1);
//           // Note: We use 0 for first receipt, 1+ for subsequent receipts
//         }
//       } catch (fetchError) {
...
//         bookingId: formData.bookingId,
//         paymentMode: formData.modeOfPayment,
//         amount: parseFloat(parseFloat(formData.amount).toFixed(2)), // Ensure 2 decimal places
//         remark: formData.remark,
//         transactionReference: formData.transactionReference
//       };
...
//         case 'Finance Disbursement':
//           paymentData.financer = bookingData?.payment?.financer?._id || bookingData?.payment?.financer;
//           paymentData.gcAmount = parseFloat(parseFloat(formData.gcAmount).toFixed(2)); // Ensure 2 decimal places
//           paymentData.amountToBeCredited = parseFloat(parseFloat(formData.amountToBeCredited).toFixed(2)); // Ensure 2 decimal places
//           break;
//         case 'Exchange':
//         case 'Pay Order':
//           paymentData.bank = formData.bank;
