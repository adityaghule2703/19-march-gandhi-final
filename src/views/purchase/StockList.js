...
//   // New states for Subdealer functionality in Export Report
//   const [selectedLocationType, setSelectedLocationType] = useState(''); // 'branch' or 'subdealer'
...
//       // Extract subdealers from response
...
//     setExportReportError('Please select a location type (Branch or Subdealer)');
//     return;
//   }
  
//   if (selectedLocationType === 'branch' && !selectedReportBranchId) {
//     setExportReportError('Please select a branch');
//     return;
//   }
  
//   if (selectedLocationType === 'subdealer' && !selectedSubdealerId) {
//     setExportReportError('Please select a subdealer');
//     return;
//   }

//   if (!selectedType) {
//     setExportReportError('Please select a model type');
//     return;
//   }
...
//       modelType: selectedType,
//       format: 'excel',
//       locationType: selectedLocationType // ADD THIS LINE - Pass the location type
//     });
    
//     // Add appropriate parameters based on location type
//     if (selectedLocationType === 'branch') {
//       params.append('branchId', selectedReportBranchId);
//     } else if (selectedLocationType === 'subdealer') {
//       // Only add subdealerId if not "all" (all subdealers)
...
//     let locationName = '';
//     if (selectedLocationType === 'branch') {
//       locationName = branches.find(b => b._id === selectedReportBranchId)?.name || 'Branch';
//     } else {
//       if (selectedSubdealerId === 'all') {
//         locationName = 'All_Subdealers';
//       } else {
//         locationName = subdealers.find(s => s._id === selectedSubdealerId)?.name || 'Subdealer';
//       }
//     }
    
//     const fileName = `Stock_Report_${locationName}_${selectedType}_${new Date().toLocaleDateString()}.xlsx`;
//     link.setAttribute('download', fileName);
...
//       {/* Export Report Modal - Updated with Subdealer option */}
//       <CModal alignment="center" visible={exportReportModalOpen} onClose={resetExportReportModal} size="lg">
//         <CModalHeader>
//           <CModalTitle>
//             <CIcon icon={cilFile} className="me-2" />
//             Export Stock Report
...
//           {/* Location Type Selection - Branch or Subdealer */}
//           <div className="mb-3">
...
//         setExportReportError('');
//       }}
//       disabled={!canExportReport || loadingSubdealers}
//     >
...
//     !selectedType || 
//     !selectedLocationType ||
//     (selectedLocationType === 'branch' && !selectedReportBranchId) ||
...
const StockList = () => {
  ...
  // New states for Subdealer functionality in Export Report
  const [selectedLocationType, setSelectedLocationType] = useState(''); // 'branch', 'subdealer', or 'both'
  const [selectedSubdealerId, setSelectedSubdealerId] = useState('');
  const [subdealers, setSubdealers] = useState([]);
  const [loadingSubdealers, setLoadingSubdealers] = useState(false);
  ...
  const handleExportReport = async () => {
    ...
    // Validate based on location type
    if (!selectedLocationType) {
      setExportReportError('Please select a location type (Branch, Subdealer, or All Territory)');
      return;
    }
    
    // For Branch selection - require branch selection (unless All Branches is selected)
    if (selectedLocationType === 'branch' && !selectedReportBranchId) {
      setExportReportError('Please select a branch');
      return;
    }
    
    // For Subdealer selection - require subdealer selection (unless All Subdealers is selected)
    if (selectedLocationType === 'subdealer' && !selectedSubdealerId) {
      setExportReportError('Please select a subdealer');
      return;
    }
    
    // For "both" (All Territory) - no additional validation needed
    if (selectedLocationType === 'both') {
      // No additional validation required for All Territory
    }
    ...
    try {
      ...
      const params = new URLSearchParams({
        modelType: selectedType,
        format: 'excel',
        locationType: selectedLocationType // This will be 'branch', 'subdealer', or 'both'
      });
      ...
      } else if (selectedLocationType === 'both') {
        // For All Territory, we don't add branchId or subdealerId
        // The API will fetch data from all branches and subdealers
      }
      ...
      let locationName = '';
      if (selectedLocationType === 'branch') {
        const selectedBranch = branches.find(b => b._id === selectedReportBranchId);
        locationName = selectedBranch?.name || (selectedReportBranchId === 'all' ? 'All_Branches' : 'Branch');
      } else if (selectedLocationType === 'subdealer') {
        if (selectedSubdealerId === 'all') {
          locationName = 'All_Subdealers';
        } else {
  ...
  return (
    <div>
      ...
      {/* Export Report Modal - Updated with Subdealer and All Territory options */}
      <CModal alignment="center" visible={exportReportModalOpen} onClose={resetExportReportModal} size="lg">
        <CModalHeader>
          <CModalTitle>
            <CIcon icon={cilFile} className="me-2" />
        ...
        <CModalBody>
          ...
          {/* Location Type Selection - Branch, Subdealer, or All Territory */}
          <div className="mb-3">
            <label className="form-label">Location Type:<span className='required'>*</span></label>
            <CFormSelect
              value={selectedLocationType}
          ...
          {selectedLocationType === 'both' && (
            <div className="alert alert-info mb-3">
              <CIcon icon={cilInfo} className="me-2" />
              Exporting stock report for all territories (Branches + Subdealers combined)
            </div>
          )}
        </CModalBody>
        <CModalFooter>
...
//   // New states for subdealer filter in Export Report
//   const [subdealers, setSubdealers] = useState([]);
...
//     fetchBranches();
//     fetchSubdealers(); // Fetch subdealers when component mounts
//   }, []);
...
//   // Fetch subdealers for the export report modal
//   const fetchSubdealers = async () => {
//     try {
...
//   const resetExportReportModal = () => {
//     setSelectedReportBranchId('');
//     setSelectedSubdealer(''); // Reset subdealer selection
//     setSubdealerSearchTerm(''); // Reset subdealer search term
//     setExportReportError('');
//     setExportReportModalOpen(false);
//   };

//   // Close subdealer dropdown when clicking outside
//   useEffect(() => {
...
//   // Reset subdealer selection when branch is selected
//   useEffect(() => {
//     if (selectedReportBranchId) {
...
//   // Validate that either branch or subdealer is selected
//   if (!selectedReportBranchId && !selectedSubdealer) {
//     setExportReportError('Please select either a branch or a subdealer');
//     return;
//   }
...
//     else if (selectedReportBranchId) {
//       params.append('locationType', 'BRANCH');
//       params.append('branchId', selectedReportBranchId);
//     }
...
//     if (selectedSubdealer) {
//       if (selectedSubdealer === 'all') {
//         sourceName = 'All_Subdealers';
//       } else {
//         const subdealer = subdealers.find(s => s._id === selectedSubdealer);
//         sourceName = subdealer?.name || 'Subdealer';
//       }
//     } else if (selectedReportBranchId) {
//       sourceName = branches.find(b => b._id === selectedReportBranchId)?.name || 'Branch';
//     }
    
//     const currentDate = formatDate(new Date());
//     const fileName = `Stock_Report_${sourceName}_${currentDate}.xlsx`;
//     link.setAttribute('download', fileName);
...
//       {/* Export Report Modal with Subdealer Filter */}
//       <CModal alignment="center" visible={exportReportModalOpen} onClose={resetExportReportModal}>
//         <CModalHeader>
//           <CModalTitle>
//             <CIcon icon={cilFile} className="me-2" />
...
//                 setExportReportError('');
//               }}
//               disabled={!canExportReport || selectedSubdealer !== ''}
//             >
//               <option value="">-- Select Branch --</option>
//               {hasAllBranchAccess && (
//                 <option value="all">All Branches</option>
...
//           {/* Subdealer Filter */}
//           <div className="mb-3 subdealer-dropdown-container" style={{ position: 'relative' }}>
//             <label className="form-label">Select Subdealer:</label>
//             {subdealerLoading ? (
//               <div>
//                 <CSpinner size="sm" />
//                 <span className="ms-2">Loading subdealers...</span>
//               </div>
//             ) : (
...
//                   disabled={selectedReportBranchId !== ''}
//                 />
//                 {showSubdealerDropdown && (
//                   <div style={{
//                     position: 'absolute',
...
//             className="submit-button"
//             onClick={handleExportReport}
//             disabled={(!selectedReportBranchId && !selectedSubdealer) || !canExportReport || exportReportLoading}
//           >
//             {exportReportLoading ? (
//               <>
//                 <CSpinner size="sm" className="me-2" />
