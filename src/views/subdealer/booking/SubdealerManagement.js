...
const SubdealerManagement = () => {
  ...
  // Debug: Log permissions to see what you actually have
  useEffect(() => {
    console.log('=== PERMISSION DEBUG ===');
    console.log('All permissions:', permissions);
    
    // Filter for Subdealer Management permissions
    const subdealerManagementPerms = permissions?.filter(p => 
      p.module === 'SUBDEALER MANAGEMENT' || 
      p.module === MODULES.SUBDEALER_MANAGEMENT
    );
    console.log('Subdealer Management permissions:', subdealerManagementPerms);
    
    // Check specific permissions
    console.log('Checking for REJECTED DISCOUNT tab permission:');
    const hasRejectedDiscountDebug = permissions?.some(p => 
      p.module === 'SUBDEALER MANAGEMENT' && 
      p.page === 'Subdealer Management' && 
      p.tab === 'REJECTED DISCOUNT' && 
      p.action === 'VIEW'
    );
    console.log('Has REJECTED DISCOUNT VIEW:', hasRejectedDiscountDebug);
    
    console.log('Checking for CANCELLED BOOKING tab permission:');
    const hasCancelledBookingDebug = permissions?.some(p => 
      p.module === 'SUBDEALER MANAGEMENT' && 
      p.page === 'Subdealer Management' && 
      p.tab === 'CANCELLED BOOKING' && 
      p.action === 'VIEW'
    );
    console.log('Has CANCELLED BOOKING VIEW:', hasCancelledBookingDebug);
    
    console.log('Checking for REJECTED CANCELLED BOOKING tab permission:');
    const hasRejectedCancelledBookingDebug = permissions?.some(p => 
      p.module === 'SUBDEALER MANAGEMENT' && 
      p.page === 'Subdealer Management' && 
      p.tab === 'REJECTED CANCELLED BOOKING' && 
      p.action === 'VIEW'
    );
    console.log('Has REJECTED CANCELLED BOOKING VIEW:', hasRejectedCancelledBookingDebug);
  ...
  // Define page name for this component (use the exact name from PAGES)
  const pageName = PAGES.SUBDEALER_MANAGEMENT.SUBDEALER_MANAGEMENT;
  
  // Tab-level permission checks - Use the correct module name
  const hasRejectedDiscountPermission = canViewTab(
    permissions, 
    MODULES.SUBDEALER_MANAGEMENT,  // Changed to SUBDEALER_MANAGEMENT
    pageName, 
    TABS.SUBDEALER_MANAGEMENT.REJECTED_DISCOUNT
  );
  
  const hasCancelledBookingPermission = canViewTab(
    permissions, 
    MODULES.SUBDEALER_MANAGEMENT,  // Changed to SUBDEALER_MANAGEMENT
    pageName, 
  ...
  const hasRejectedCancelledBookingPermission = canViewTab(
    permissions, 
    MODULES.SUBDEALER_MANAGEMENT,  // Changed to SUBDEALER_MANAGEMENT
    pageName, 
  ...
  // Page-level permission check - user needs at least ONE tab permission
  const hasViewPermission = hasRejectedDiscountPermission || 
                           hasCancelledBookingPermission || 
                           hasRejectedCancelledBookingPermission;
  
  // Log the permission results
  console.log('Permission check results:');
  console.log('- hasRejectedDiscountPermission:', hasRejectedDiscountPermission);
  console.log('- hasCancelledBookingPermission:', hasCancelledBookingPermission);
  console.log('- hasRejectedCancelledBookingPermission:', hasRejectedCancelledBookingPermission);
  console.log('- hasViewPermission:', hasViewPermission);
  
  // Tab-specific permission checks for actions
  const hasRejectedDiscountActions = canUpdateInPage(
    permissions, 
    MODULES.SUBDEALER_MANAGEMENT,  // Changed to SUBDEALER_MANAGEMENT
    pageName, 
    TABS.SUBDEALER_MANAGEMENT.REJECTED_DISCOUNT
  ...
  const hasCancelledBookingActions = canUpdateInPage(
    permissions, 
    MODULES.SUBDEALER_MANAGEMENT,  // Changed to SUBDEALER_MANAGEMENT
    pageName, 
    TABS.SUBDEALER_MANAGEMENT.CANCELLED_BOOKING
  ...
  const hasRejectedCancelledBookingActions = canUpdateInPage(
    permissions, 
    MODULES.SUBDEALER_MANAGEMENT,  // Changed to SUBDEALER_MANAGEMENT
    pageName, 
    TABS.SUBDEALER_MANAGEMENT.REJECTED_CANCELLED_BOOKING
  ...
  useEffect(() => {
    if (!hasViewPermission) {
      console.log('No permission to view Subdealer Management');
      showError('You do not have permission to view Subdealer Management');
      return;
    }
  ...
  const handleOpenRestoreModal = (bookingId, type, tabType) => {
    ...
    if (tabType === 'rejected_discount') {
      ...
      hasPermission = hasRejectedCancelledBookingActions;
    }
  ...
  const renderManagementTable = (records, tabIndex) => {
    ...
    const showRestoreActionsColumn = 
      (isRejectedDiscountTab && hasRejectedDiscountActions) || 
      (isCancelledBookingTab && hasCancelledBookingActions) ||
      (isRejectedCancelledBookingTab && hasRejectedCancelledBookingActions);
