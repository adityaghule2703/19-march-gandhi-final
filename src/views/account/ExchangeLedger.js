...
// const generateReceiptHTML = (transaction, transactionData, qrCodeImage, receivedDate, currentDate, brokerName, branchName, branchAddress, branchPhone, branchEmail) => {
//   const receiptNumber = transaction.receiptNumber || 'N/A';
//   const amount = transaction.amount || 0;
//   const paymentMode = transaction.modeOfPayment || 'Cash';
//   const referenceNumber = transaction.referenceNumber || '';
//   const approvalStatus = transaction.approvalStatus || 'Pending';
...
//     .remark-box strong { color: #495057; }
//     @page { size: A4; margin: 10mm; }
//     @media print { body { padding: 0; } .receipt-copy { page-break-inside: avoid; } }
//   </style>
// </head>
...
//                                 </CDropdownToggle>
//                                 <CDropdownMenu>
//                                // For broker summary row (around line 937)
// {sortedTransactions.map((transaction, txIndex) => {
//   const transactionId = transaction._id;
//   const transactionAmount = transaction.amount || 0;
//   const transactionDate = transaction.dateFormatted || new Date(transaction.date || transaction.createdAt).toLocaleDateString('en-GB');
...
//         <div>
//           <div><strong>Receipt #{txIndex + 1}</strong> - <small className="text-muted">{receiptNumber}</small></div>
//           <small>₹{transactionAmount.toLocaleString('en-IN')} - {transactionDate}</small>
//           <div><small className="text-muted">{remark}</small></div>
//         </div>
//       </div>
//     </CDropdownItem>
//   );
...
const ExchangeLedger = () => {
  ...
  // On Account Export modal states
  const [showOnAccountExportModal, setShowOnAccountExportModal] = useState(false);
  const [onAccountExportStartDate, setOnAccountExportStartDate] = useState(null);
  const [onAccountExportEndDate, setOnAccountExportEndDate] = useState(null);
  const [onAccountExportBranchId, setOnAccountExportBranchId] = useState('');
  const [onAccountExportError, setOnAccountExportError] = useState('');
  const [onAccountExportLoading, setOnAccountExportLoading] = useState(false);
  
  // Commission Report Export modal states
  const [showCommissionExportModal, setShowCommissionExportModal] = useState(false);
  const [commissionExportStartDate, setCommissionExportStartDate] = useState(null);
  const [commissionExportEndDate, setCommissionExportEndDate] = useState(null);
  const [commissionExportBranchId, setCommissionExportBranchId] = useState('');
  const [commissionExportError, setCommissionExportError] = useState('');
  const [commissionExportLoading, setCommissionExportLoading] = useState(false);
  const [commissionExportBrokerId, setCommissionExportBrokerId] = useState('');
  const [commissionExportBrokerBranches, setCommissionExportBrokerBranches] = useState([]);
...
<body>
  ...
  // On Account Export handlers
  const handleOpenOnAccountExportModal = () => {
    if (!canCreateExchangeLedger) {
      showError('You do not have permission to export reports');
      return;
    }
    setShowOnAccountExportModal(true);
    setOnAccountExportError('');
    if (isFiltered && selectedBranch) {
      setOnAccountExportBranchId(selectedBranch);
    }
  };

  const handleCloseOnAccountExportModal = () => {
    setShowOnAccountExportModal(false);
    setOnAccountExportStartDate(null);
    setOnAccountExportEndDate(null);
    setOnAccountExportBranchId('');
    setOnAccountExportError('');
    setOnAccountExportLoading(false);
  };

  const handleOnAccountExcelExport = async () => {
    if (!canCreateExchangeLedger) {
      showError('You do not have permission to export reports');
      return;
    }
    ...
    if (!onAccountExportBranchId) {
      setOnAccountExportError('Please select a branch');
      return;
    }

    if (!onAccountExportStartDate || !onAccountExportEndDate) {
      setOnAccountExportError('Please select both start and end dates');
      return;
    }

    if (onAccountExportStartDate > onAccountExportEndDate) {
      setOnAccountExportError('Start date cannot be after end date');
      return;
    }

    try {
      setOnAccountExportLoading(true);
      const formattedStartDate = formatDateForAPI(onAccountExportStartDate);
      const formattedEndDate = formatDateForAPI(onAccountExportEndDate);

      const params = new URLSearchParams({
        branchId: onAccountExportBranchId,
        startDate: formattedStartDate,
        endDate: formattedEndDate
      });

      const response = await axiosInstance.get(`/reports/broker/on-account?${params.toString()}`, { responseType: 'blob' });
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        const errorData = JSON.parse(text);
        if (!errorData.success && errorData.message) {
          setOnAccountExportError(errorData.message);
          Swal.fire({ icon: 'error', title: 'Export Failed', text: errorData.message });
          return;
        }
      }
      ...
      const link = document.createElement('a');
      link.href = url;
      const branchName = branches.find(b => b._id === onAccountExportBranchId)?.name || 'Branch';
      const startDateStr = formatDateDDMMYYYY(onAccountExportStartDate);
      const endDateStr = formatDateDDMMYYYY(onAccountExportEndDate);
      const fileName = `Broker_OnAccount_Report_${branchName}_${startDateStr}_to_${endDateStr}.xlsx`;
      link.setAttribute('download', fileName);
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(url);
      Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'On Account Report exported successfully!', showConfirmButton: false, timer: 3000, timerProgressBar: true });
      handleCloseOnAccountExportModal();
    } catch (error) {
      console.error('Error exporting on account report:', error);
      if (error.response && error.response.data instanceof Blob) {
        try {
          const text = await new Promise((resolve, reject) => {
            const reader = new FileReader();
          ...
          setOnAccountExportError('Failed to export report');
          Swal.fire({ icon: 'error', title: 'Export Failed', text: 'Failed to export report' });
        }
      } else if (error.response?.data?.message) {
        setOnAccountExportError(error.response.data.message);
        Swal.fire({ icon: 'error', title: 'Export Failed', text: error.response.data.message });
      } else if (error.message) {
        setOnAccountExportError(error.message);
        Swal.fire({ icon: 'error', title: 'Export Failed', text: error.message });
      } else {
        setOnAccountExportError('Failed to export report');
        Swal.fire({ icon: 'error', title: 'Export Failed', text: 'Failed to export report' });
  ...
  // Commission Report Export handlers
  const handleOpenCommissionExportModal = () => {
    if (!canCreateExchangeLedger) {
      showError('You do not have permission to export reports');
      return;
    }
    setShowCommissionExportModal(true);
    setCommissionExportError('');
    // Reset selections
    setCommissionExportBrokerId('');
    setCommissionExportBranchId('');
    setCommissionExportBrokerBranches([]);
    setCommissionExportStartDate(null);
    setCommissionExportEndDate(null);
  };

  const handleCloseCommissionExportModal = () => {
    setShowCommissionExportModal(false);
    setCommissionExportStartDate(null);
    setCommissionExportEndDate(null);
    setCommissionExportBranchId('');
    setCommissionExportBrokerId('');
    setCommissionExportBrokerBranches([]);
    setCommissionExportError('');
    setCommissionExportLoading(false);
  };

  // Function to handle broker selection change for commission report
  const handleCommissionBrokerChange = (brokerId) => {
    setCommissionExportBrokerId(brokerId);
    setCommissionExportBranchId(''); // Reset branch when broker changes
    
    if (brokerId) {
      // Find selected broker from the brokers list
      const selectedBroker = brokers.find(b => b._id === brokerId);
      if (selectedBroker && selectedBroker.branches) {
        // Extract branch details
        const branchesList = selectedBroker.branches.map(b => ({
          _id: b.branch._id,
          name: b.branch.name
        }));
        setCommissionExportBrokerBranches(branchesList);
      } else {
        setCommissionExportBrokerBranches([]);
      }
    } else {
  ...
  const handleCommissionExcelExport = async () => {
    if (!canCreateExchangeLedger) {
      showError('You do not have permission to export reports');
      return;
    }
    ...
    if (!commissionExportStartDate || !commissionExportEndDate) {
      setCommissionExportError('Please select both start and end dates');
      return;
    }

    if (commissionExportStartDate > commissionExportEndDate) {
      setCommissionExportError('Start date cannot be after end date');
      return;
    }

    try {
      setCommissionExportLoading(true);
      const formattedStartDate = formatDateForAPI(commissionExportStartDate);
      const formattedEndDate = formatDateForAPI(commissionExportEndDate);

      let url = `/reports/broker/commissionReport?startDate=${formattedStartDate}&endDate=${formattedEndDate}`;
      
      // Add broker filter if selected
      if (commissionExportBrokerId) {
        url += `&brokerId=${commissionExportBrokerId}`;
      }
      
      // Add branch filter if selected
      if (commissionExportBranchId) {
        url += `&branchId=${commissionExportBranchId}`;
      }
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        const errorData = JSON.parse(text);
        if (!errorData.success && errorData.message) {
          setCommissionExportError(errorData.message);
          Swal.fire({ icon: 'error', title: 'Export Failed', text: errorData.message });
          return;
        }
      }
      ...
      const selectedBroker = brokers.find(b => b._id === commissionExportBrokerId);
      const brokerName = selectedBroker?.name || 'All_Brokers';
      const branchName = commissionExportBranchId 
        ? (commissionExportBrokerBranches.find(b => b._id === commissionExportBranchId)?.name || branches.find(b => b._id === commissionExportBranchId)?.name || 'Branch')
        : 'All_Branches';
      const startDateStr = formatDateDDMMYYYY(commissionExportStartDate);
      const endDateStr = formatDateDDMMYYYY(commissionExportEndDate);
      const fileName = `Commission_Report_${brokerName}_${branchName}_${startDateStr}_to_${endDateStr}.xlsx`;
      link.setAttribute('download', fileName);
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(url_blob);
      Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'Commission Report exported successfully!', showConfirmButton: false, timer: 3000, timerProgressBar: true });
      handleCloseCommissionExportModal();
    } catch (error) {
      ...
      if (error.response && error.response.data instanceof Blob) {
        try {
          ...
          setCommissionExportError('Failed to export report');
          Swal.fire({ icon: 'error', title: 'Export Failed', text: 'Failed to export report' });
        }
      } else if (error.response?.data?.message) {
        setCommissionExportError(error.response.data.message);
        Swal.fire({ icon: 'error', title: 'Export Failed', text: error.response.data.message });
      } else if (error.message) {
        setCommissionExportError(error.message);
        Swal.fire({ icon: 'error', title: 'Export Failed', text: error.message });
      } else {
        setCommissionExportError('Failed to export report');
        Swal.fire({ icon: 'error', title: 'Export Failed', text: 'Failed to export report' });
...
const renderBrokerPaymentTab = () => {
  ...
  return (
    <div>
          ...
          <div>
            {activeTab === TAB.BROKER_LEDGER && (
              <>
                ...
                {canCreateExchangeLedger && (
                  <>
                    <CButton size="sm" className="action-btn me-1" onClick={handleOpenExportModal} title="Export Broker Payment Report">
                      <FontAwesomeIcon icon={faFileExcel} className='me-1' /> Broker Payment Report
                    </CButton>
                    <CButton size="sm" className="action-btn me-1" onClick={handleOpenOnAccountExportModal} title="Export On Account Report">
                      <FontAwesomeIcon icon={faFileExcel} className='me-1' /> On Account Report
                    </CButton>
                    <CButton size="sm" className="action-btn me-1" onClick={handleOpenCommissionExportModal} title="Export Commission Report">
                      <FontAwesomeIcon icon={faFileExcel} className='me-1' /> Commission Report
                    </CButton>
                  </>
                )}
              </>
      ...
      {/* Export Broker Payment Report Modal */}
      <CModal alignment="center" visible={showExportModal} onClose={handleCloseExportModal} size="lg">
        <CModalHeader><CModalTitle><FontAwesomeIcon icon={faCalendarAlt} className="me-2" /> Export Broker Payment Report</CModalTitle></CModalHeader>
        <CModalBody>
          {exportError && <CAlert color="warning" className="mb-3">{exportError}</CAlert>}
          <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={enIN}>
            <div className="mb-3">
      ...
      {/* Export On Account Report Modal */}
      <CModal alignment="center" visible={showOnAccountExportModal} onClose={handleCloseOnAccountExportModal} size="lg">
        <CModalHeader><CModalTitle><FontAwesomeIcon icon={faCalendarAlt} className="me-2" /> Export On Account Report</CModalTitle></CModalHeader>
        <CModalBody>
          {onAccountExportError && <CAlert color="warning" className="mb-3">{onAccountExportError}</CAlert>}
          <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={enIN}>
            <div className="mb-3">
              <CFormLabel>Select Branch</CFormLabel>
              <CFormSelect value={onAccountExportBranchId} onChange={(e) => { setOnAccountExportBranchId(e.target.value); setOnAccountExportError(''); }} disabled={!canCreateExchangeLedger}>
                <option value="">-- Select Branch --</option>
                <option value="ALL">All Branches</option>
                {branches.map((branch) => (
                  <option key={branch._id} value={branch._id}>{branch.name || 'N/A'}</option>
            ...
            <div className="mb-3">
              <DatePicker label="Start Date" value={onAccountExportStartDate} onChange={(newValue) => { setOnAccountExportStartDate(newValue); setOnAccountExportError(''); }} renderInput={(params) => <TextField {...params} fullWidth size="small" />} inputFormat="dd/MM/yyyy" mask="__/__/____" views={['day', 'month', 'year']} disabled={!canCreateExchangeLedger} />
            </div>
            <div className="mb-3">
              <DatePicker label="End Date" value={onAccountExportEndDate} onChange={(newValue) => { setOnAccountExportEndDate(newValue); setOnAccountExportError(''); }} renderInput={(params) => <TextField {...params} fullWidth size="small" />} inputFormat="dd/MM/yyyy" mask="__/__/____" minDate={onAccountExportStartDate} views={['day', 'month', 'year']} disabled={!canCreateExchangeLedger} />
            </div>
          </LocalizationProvider>
        </CModalBody>
        <CModalFooter>
          <CButton color="secondary" onClick={handleCloseOnAccountExportModal}>Cancel</CButton>
          <CButton className="submit-button" onClick={handleOnAccountExcelExport} disabled={!onAccountExportStartDate || !onAccountExportEndDate || !onAccountExportBranchId || !canCreateExchangeLedger || onAccountExportLoading}>
            {onAccountExportLoading ? <><CSpinner size="sm" className="me-2" /> Exporting...</> : 'Export Excel'}
          </CButton>
        </CModalFooter>
      </CModal>
      
      {/* Export Commission Report Modal */}
      <CModal alignment="center" visible={showCommissionExportModal} onClose={handleCloseCommissionExportModal} size="lg">
        <CModalHeader><CModalTitle><FontAwesomeIcon icon={faCalendarAlt} className="me-2" /> Export Commission Report</CModalTitle></CModalHeader>
        <CModalBody>
          {commissionExportError && <CAlert color="warning" className="mb-3">{commissionExportError}</CAlert>}
          <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={enIN}>
            <div className="mb-3">
              <CFormLabel>Select Broker</CFormLabel>
              <CFormSelect value={commissionExportBrokerId} onChange={(e) => handleCommissionBrokerChange(e.target.value)} disabled={!canCreateExchangeLedger}>
            ...
            <div className="mb-3">
              <CFormLabel>Select Branch</CFormLabel>
              <CFormSelect 
                value={commissionExportBranchId} 
                onChange={(e) => { setCommissionExportBranchId(e.target.value); setCommissionExportError(''); }} 
                disabled={!canCreateExchangeLedger || !commissionExportBrokerId || commissionExportBrokerBranches.length === 0}
              >
                <option value="">All Branches</option>
                {commissionExportBrokerBranches.map((branch) => (
                  <option key={branch._id} value={branch._id}>{branch.name}</option>
                ))}
              </CFormSelect>
              {commissionExportBrokerId && commissionExportBrokerBranches.length === 0 && (
                <small className="text-muted">No branches found for selected broker</small>
              )}
            </div>
            <div className="mb-3">
              <DatePicker 
                label="Start Date" 
                value={commissionExportStartDate} 
                onChange={(newValue) => { setCommissionExportStartDate(newValue); setCommissionExportError(''); }} 
                renderInput={(params) => <TextField {...params} fullWidth size="small" />} 
                inputFormat="dd/MM/yyyy" 
                mask="__/__/____" 
                views={['day', 'month', 'year']} 
            ...
            <div className="mb-3">
              <DatePicker 
                label="End Date" 
                value={commissionExportEndDate} 
                onChange={(newValue) => { setCommissionExportEndDate(newValue); setCommissionExportError(''); }} 
                renderInput={(params) => <TextField {...params} fullWidth size="small" />} 
                inputFormat="dd/MM/yyyy" 
                mask="__/__/____" 
                minDate={commissionExportStartDate} 
                views={['day', 'month', 'year']} 
                disabled={!canCreateExchangeLedger} 
              />
            </div>
        ...
        <CModalFooter>
          <CButton color="secondary" onClick={handleCloseCommissionExportModal}>Cancel</CButton>
          <CButton className="submit-button" onClick={handleCommissionExcelExport} disabled={!commissionExportStartDate || !commissionExportEndDate || !canCreateExchangeLedger || commissionExportLoading}>
            {commissionExportLoading ? <><CSpinner size="sm" className="me-2" /> Exporting...</> : 'Export Excel'}
          </CButton>
        </CModalFooter>
      </CModal>
