...
import CIcon from '@coreui/icons-react';
import { cilCloudUpload, cilPaperclip, cilFile, cilInfo } from '@coreui/icons';
import { 
  MODULES, 
  PAGES,
  ACTIONS,
...
const UploadChallan = () => {
  ...
  // Handle viewing challan
  const handleViewChallan = async (transfer) => {
    setIsLoadingChallan(true);
    
    try {
      // Get vehicle items from the transfer
      const items = transfer.items || [];
      const vehicles = items.map(item => item.vehicle).filter(Boolean);
      
      // Map the transfer data to the format expected by TransferChallan
      const challanData = {
        transferDetails: {
          createdAt: transfer.createdAt || transfer.transferDate || new Date()
        },
        fromType: 'branch',
        fromBranch: transfer.fromBranchDetails || null,
        fromSubdealer: transfer.fromSubdealerDetails || null,
        toType: transfer.toBranchDetails ? 'branch' : 'subdealer',
        toBranch: transfer.toBranchDetails || null,
        toSubdealer: transfer.toSubdealerDetails || null,
        vehicles: vehicles,
        destinationName: transfer.toBranchDetails?.name || transfer.toSubdealerDetails?.name || ''
      };

      // Set from type based on what's available
      if (transfer.fromBranchDetails) {
        challanData.fromType = 'branch';
        challanData.fromBranch = transfer.fromBranchDetails;
        challanData.fromSubdealer = null;
      } else if (transfer.fromSubdealerDetails) {
        challanData.fromType = 'subdealer';
        challanData.fromSubdealer = transfer.fromSubdealerDetails;
        challanData.fromBranch = null;
      }

      // Set to type based on what's available
      if (transfer.toBranchDetails) {
        challanData.toType = 'branch';
        challanData.toBranch = transfer.toBranchDetails;
        challanData.toSubdealer = null;
      } else if (transfer.toSubdealerDetails) {
        challanData.toType = 'subdealer';
        challanData.toSubdealer = transfer.toSubdealerDetails;
        challanData.toBranch = null;
      }

      setChallanData(challanData);
      setShowChallanModal(true);
    } catch (error) {
      console.error('Error preparing challan data:', error);
      showError('Failed to load challan details');
    } finally {
                            ...
                            <CTableDataCell rowSpan={transfer.items.length} style={{ maxWidth: '200px' }}>
                              ...
                              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                <CButton 
                                  ...
                                  disabled={isLoadingChallan || !transfer.items || transfer.items.length === 0}
          ...
          {isLoadingChallan ? (
            <div className="d-flex justify-content-center align-items-center" style={{ minHeight: '300px' }}>
              <div className="text-center">
                ...
                <p className="mt-3 text-muted">Loading challan details...</p>
              </div>
            </div>
          ) : (
