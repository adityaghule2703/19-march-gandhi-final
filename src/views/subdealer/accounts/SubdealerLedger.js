        ...
        if (receipt.allocations && receipt.allocations.length > 0) {
          receipt.allocations.forEach((allocation) => {
            allTransactions.push({
              ...
              data: allocation,
              receipt: receipt,
              date: new Date(allocation.allocatedAt)
            });
          });
        }
      });
                    ...
                    if (transaction.type === 'receipt') {
                      ...
                      row = `
                        <tr>
                          ...
                          <td>${allocation.ledger?.transactionReference || transaction.receipt?.refNumber || ''}</td>
                          <td class="text-right">0</td>
                      ...
                      if (accessory.isDebit) {
                        ...
                        row = `
                          <tr>
                            ...
                            <td>
                              Accessory Billing - Debit<br>
                              ${accessory.remark || 'Accessory Purchase'}
                            </td>
                            <td>${accessory.transactionReference || ''}</td>
                            <td class="text-right">0</td>
                            <td class="text-right">${accessoryAmount.toLocaleString('en-IN')}</td>
                            <td class="text-right">${runningBalance.toLocaleString('en-IN')}</td>
                          </tr>
                        `;
                      } else {
                        ...
                        row = `
                          <tr>
                            <td>${new Date(accessory.createdAt).toLocaleDateString('en-GB')}</td>
                            <td>
                              Accessory Billing - Credit<br>
                              ${accessory.remark || 'Accessory Payment'}
                            </td>
                            <td>${accessory.transactionReference || ''}</td>
                            <td class="text-right">${accessoryAmount.toLocaleString('en-IN')}</td>
                            <td class="text-right">0</td>
