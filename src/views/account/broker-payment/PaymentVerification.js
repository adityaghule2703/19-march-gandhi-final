...
function PaymentVerification() {
  ...
  const canViewPaymentVerification = hasSafePagePermission(
    permissions, 
    MODULES.ACCOUNT, 
    PAGES.ACCOUNT.BROKER_PAYMENT_VERIFICATION, 
    ACTIONS.VIEW
  ...
  // For verifying payments, use ONLY CREATE permission for PENDING VERIFICATION tab
  const canCreatePendingVerification = hasSafePagePermission(
    permissions, 
    MODULES.ACCOUNT, 
    PAGES.ACCOUNT.BROKER_PAYMENT_VERIFICATION, 
  ...
  const handleVerifyPayment = async (entry) => {
    // Use ONLY CREATE permission for verification on PENDING VERIFICATION tab
    if (!canCreatePendingVerification) {
      showError('You do not have permission to verify payments');
      return;
    }
