...
//       // If user is a subdealer, automatically set the filter to their own subdealer
//       if (isSubdealer && userSubdealerId) {
//         setSelectedSubdealer(userSubdealerId);
//         fetchCommissionData(userSubdealerId);
//       }
//     } catch (error) {
//       const message = showError(error);
//       if (message) {
...
//     try {
//       const response = await axiosInstance.get(
//         `/commission-master/export-template?subdealer_id=${selectedSubdeaker}&model_type=${selectedModelType}`,
//         {
//           responseType: 'blob'
//         }
//       );
...
//                   Subdealers can only view their own commission data
//                 </div>
//               </div>
//             ) : (
//               <CFormSelect 
...
//         </CModalBody>
//         <CModalFooter>
//           <CButton className='submit-button' onClick={handleApplyFilter} disabled={!canViewCommission || (isSubdealer && !selectedSubdealer)}>
//             Apply
//           </CButton>
//         </CModalFooter>
//       </CModal>
...
//                 />
//                 <div className="text-muted small">
//                   Subdealers can only apply commission to their own account
//                 </div>
//               </div>
//             ) : (
//               <CFormSelect 
...
//                 />
//                 <div className="text-muted small">
//                   Subdealers can only export templates for their own account
//                 </div>
//               </div>
//             ) : (
//               <CFormSelect 
...
//                 />
//                 <div className="text-muted small">
//                   Subdealers can only import data for their own account
//                 </div>
//               </div>
//             ) : (
//               <CFormSelect 
...
const CommissionList = () => {
  ...
  const handleExportCSV = async () => {
    // Exporting CSV should also check CREATE permission
    if (!canCreateCommission) {
      showError('You do not have permission to export commission template');
      return;
    }
  ...
  return (
    <div className="form-container">
      ...
      <CCard className="table-container mt-4">
        <CCardHeader className='card-header d-flex justify-content-between align-items-center'>
          <div>
            ...
            {/* Export button - Now also checks CREATE permission */}
            <CButton 
              size="sm" 
              className="action-btn me-1"
              onClick={() => setExportModalVisible(true)}
      ...
      <CModal visible={exportModalVisible} onClose={() => setExportModalVisible(false)}>
        ...
        <CModalFooter>
          <CButton className='submit-button' onClick={handleExportCSV} disabled={!canCreateCommission}>
            Generate CSV
          </CButton>
        </CModalFooter>
      </CModal>
