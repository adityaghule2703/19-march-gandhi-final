...
import { Link } from 'react-router-dom';
import CIcon from '@coreui/icons-react';
import { cilCloudUpload, cilZoom, cilSettings, cilList, cilFile, cilCloudDownload } from '@coreui/icons';
import { axiosInstance, getDefaultSearchFields, showError, showSuccess, useTableFilter } from '../../utils/tableImports';
import '../../css/invoice.css';
import '../../css/table.css';
import KYCDocuments from './KYCDocuments';
...
function RTOPaper() {
    ...
    // New function to handle Excel export of pending bookings
    const handleExportPendingBookings = async() => {
        try {
            ...
            // Call the new API endpoint to fetch pending bookings for export
            const queryString = params.toString();
            const apiUrl = queryString ? `/bookings/export-kyc-pending?${queryString}` : '/bookings/export-kyc-pending';
            ...
            let filename = 'pending-kyc-bookings.xlsx';
            ...
            showSuccess('Pending bookings exported successfully!');
        } catch (error) {
            const errorMessage = error.response?.data?.message || error.message || 'Failed to export pending bookings';
            showError(errorMessage);
            if (errorMessage) {
                setError(errorMessage);
            }
        ...
        return (
            ...
            {filteredPendings.length === 0 ? (
                    ...
                    filteredPendings.map((rtoItem, index) => (
                        ...
                        {!rtoItem.kycStatus || rtoItem.kycStatus === 'NOT_UPLOADED' || rtoItem.kycStatus === 'REJECTED' ? (
                                canCreateInRtoPaperPendingTab ? (
                                    <Link to={`/upload-kyc-rto/${rtoItem.bookingId?.id}`}
                                    state={{
                                            bookingId: rtoItem.bookingId?.id,
                                            customerName: rtoItem.bookingId?.customerName,
                                            address: `${rtoItem.bookingId?.customerAddress || ''}`,
                        ...
                        {canViewInRtoPaperPendingTab && (
                                <>
                                <CTableDataCell>
                                <CButton size="sm" className="action-btn" onClick={() => handleViewKYC(rtoItem)}>
                                <CIcon icon={cilZoom} className="me-1" />
                                View
                                </CButton>
                                </CTableDataCell>
                                <CTableDataCell>
                                <CButton size="sm" color="info" className="action-btn" onClick={() => handleVahanFeeding(rtoItem)} disabled={vahanLoading}>
                                <CIcon icon={cilSettings} className="me-1" />
                                Vahan Feeding
                                </CButton>
                                </CTableDataCell>
            ...
            {filteredApproved.length === 0 ? (
                    ...
                    filteredApproved.map((rtoItem, index) => (
                        ...
                        {rtoItem.displayStatus?.paper === 'Verified' ? (
                                ...
                                canCreateInCompletedTab ? (
                                    <CButton size="sm" color="success" onClick={() => handleApprovePaper(rtoItem._id)} disabled={actionLoadingId === rtoItem._id}>
                                    {actionLoadingId === rtoItem._id ? (
                                            <>
                                            <CSpinner size="sm" className="me-1" />
                                            Processing...
    ...
    return (
        ...
        {canViewAnyTab ? (
            ...
            <div className="d-flex align-items-center justify-content-between mb-3 border-bottom">
                ...
                {/* Export Excel section - placed right after tabs */}
                <div className="d-flex align-items-center gap-3">
                    <div className="d-flex align-items-center gap-2">
                        <CFormLabel className="mb-0" style={{ whiteSpace: 'nowrap' }}>Start:</CFormLabel>
                        <CFormInput
                            type="date"
                            style={{ height: '32px', width: '150px', borderRadius: '4px' }}
                            value={exportStartDate}
                            onChange={(e) => setExportStartDate(e.target.value)}
                        />
                    </div>
                    <div className="d-flex align-items-center gap-2">
                        <CFormLabel className="mb-0" style={{ whiteSpace: 'nowrap' }}>End:</CFormLabel>
                        <CFormInput
                            type="date"
                            style={{ height: '32px', width: '150px', borderRadius: '4px' }}
                            value={exportEndDate}
                    ...
                    <CButton
                        ...
                        onClick={handleExportPendingBookings}
        ...
        {vahanError && (
            <CAlert color="danger">{vahanError}</CAlert>
        )}
        ...
        {vahanData && (
            ...
            {vahanData.payment?.isFinance && (
                <>
                <div>
                <strong>HPA: </strong>{vahanData.payment?.hpa?.applicable ? (
                <span className="text-success ms-2">✓Applicable</span>
                ) : (
                <span className="text-danger fw-bold ms-2">NO HPA</span>
                )}
                </div>
                {vahanData.payment?.hpa?.applicable && vahanData.payment?.hpa?.financer && (
                <div>
        ...
        {vahanData && vahanData.eligibility?.canFeedToVAHAN && (
            <CButton color="success" onClick={handleFeedToVahan} disabled={feedingLoading}>
            {feedingLoading ? (
                <>
                <CSpinner size="sm" className="me-1" />
                Feeding...
