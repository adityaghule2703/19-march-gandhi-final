...
// //     console.log('Booking data rtoCode:', bookingData.rtoCode); // Add this log
...
// //     console.log('RTO Code from API:', bookingData.rtoCode); // Add this

// //     // Set form data with subsidy amount
// //     const formDataToSet = {
...
// //     console.log('Setting form data with rto_code:', formDataToSet.rto_code); // Add this
...
// //   //   return allModelHeaders.filter((price) => {
...
// //     // ALWAYS show ACCESSORIES TOTAL header in Tab 6 (even though it's excluded from calculations)
// //     if (headerKey === 'ACCESSORIES TOTAL') {
// //       return true;
// //     }
...
// //         if (isEditMode) {
// //           // In edit mode: include headers that are in optionalComponents AND not in uncheckedHeaders
...
// //         } else {
// //           // In new booking mode: include headers that are not explicitly unchecked
// //           const isExplicitlyUnchecked = formData.uncheckedHeaders && 
// //             formData.uncheckedHeaders.includes(headerId);
// //           return isMandatory || !isExplicitlyUnchecked;
// //         }
...
// //      // Only check accessories that are SELECTED by the user
// //      // Find matching accessories for this header
// //     // Find matching accessories for this header
...
// // // Get accessory price (use highest if multiple accessories)
...
// //         const header = getSelectedModelHeaders().find(p => p.header && (p.header._id === headerId || p.header.id === headerId));
...
// //   // In edit mode, check if accessory is in selected_accessories
...
// //     {getSelectedHeadersForTab6().length > 0 && (
// //       <div className="model-headers-section" style={{ marginTop: '20px' }}>
// //         <h5>
// //           Model Options ({getSelectedHeadersForTab6().length} selected)
// //           {!formData.hpa && <span style={{ color: '#dc3545', fontSize: '0.9em', marginLeft: '10px' }}>
// //             (HPA-related options hidden as HPA is disabled)
// //           </span>}
// //         </h5>
...
// //             <CTableBody>
// //               {(() => {
// //                 // First, calculate total discounts by category
...
// //                 // First pass: collect all headers and calculate total discounts by category
// //                 getSelectedHeadersForTab6().forEach((price) => {
// //                   const header = price.header;
// //                   const headerId = header._id || header.id;
// //                   const headerKey = header.header_key || '';
...
// //                   // Categorize discounts (skip ACCESSORIES TOTAL header itself from discount calculations)
// //                   if (headerKey !== 'ACCESSORIES TOTAL' && discountAmount > 0) {
...
// //                 console.log('Discounts by category:', {
// //                   vehicle_price: totalDiscountFromVehiclePrice,
// //                   AddONservices: totalDiscountFromAddOnServices,
// //                   accessories: totalDiscountFromAccessories,
// //                   other: totalDiscountFromOtherCategories
// //                 });
                
// //                 // Second pass: render rows with adjusted calculations
// //                 return headersWithCalculations.map((item) => {
// //                   const { price, header, headerId, headerKey, categoryKey, isSpecialHeader, discountAmount } = item;
// //                   const isMandatory = header.is_mandatory;
...
// //                   // For special headers, subtract appropriate discounts based on category
...
// //                       console.log(`Adjusting ON ROAD PRICE (${headerKey}): ${finalPrice} - ${discountApplied} = ${adjustedPrice}`);
...
// //                       adjustedPrice = Math.max(0, finalPrice - discountApplied);
// //                       console.log(`Adjusting ADDON SERVICES TOTAL (${headerKey}): ${finalPrice} - ${discountApplied} = ${adjustedPrice}`);
// //                     } else if (headerKey === 'ACCESSORIES TOTAL') {
// //                       // Apply accessories discounts to ACCESSORIES TOTAL
// //                       discountApplied = totalDiscountFromAccessories;
// //                       adjustedPrice = Math.max(0, finalPrice - discountApplied);
// //                       console.log(`Adjusting ACCESSORIES TOTAL (${headerKey}): ${finalPrice} - ${discountApplied} = ${adjustedPrice}`);
// //                     }
// //                   }
...
// //                   // For special headers, use adjusted price for calculations
// //                   // For non-special headers, use regular discount calculation
// //                   let taxable, cgstAmount, sgstAmount, cgstRate, sgstRate, lineTotal;
                  
// //                   if (isSpecialHeader) {
// //                     // For special headers, the "discount" is already applied to the price
...
// //                     console.log(`Applying subsidy of ₹${subsidyAmount} to ${headerKey}. Original: ₹${lineTotal + subsidyAmount}, After subsidy: ₹${lineTotal}`);
// //                   }
...
// //                       <CTableDataCell>
// //                         {/* For special headers, show the adjusted price (after discount from appropriate categories) */}
// //                         {isSpecialHeader ? (
...
// //   console.log('All model headers count:', allModelHeaders.length);
// //   console.log('ACCESSORIES TOTAL in all headers:', allModelHeaders.find(p => p.header?.header_key === 'ACCESSORIES TOTAL'));
  
// //   // Filter to show only headers that should be displayed
// //   const filteredHeaders = allModelHeaders.filter((price) => {
// //     if (!price.header || !price.header._id) {
// //       console.log('Skipping header without ID:', price);
// //       return false;
// //     }
...
// //     // ALWAYS show ACCESSORIES TOTAL header in Tab 6
// //     if (headerKey === 'ACCESSORIES TOTAL') {
// //       console.log('Including ACCESSORIES TOTAL header');
// //       return true;
// //     }
...
// //     if (!shouldShowHeader) {
// //       console.log('Excluding HPA header:', headerKey);
// //       return false;
// //     }
...
// //       // In edit mode: show headers that are in optionalComponents (were selected in original booking)
// //       const isInOptionalComponents = formData.optionalComponents.includes(headerId);
// //       const result = isMandatory || isInOptionalComponents;
// //       console.log(`Edit mode - ${headerKey}: mandatory=${isMandatory}, inOptional=${isInOptionalComponents}, result=${result}`);
// //       return result;
// //     } else {
// //       // In new booking mode: show headers that are not explicitly unchecked
// //       const isExplicitlyUnchecked = formData.uncheckedHeaders && 
// //         formData.uncheckedHeaders.includes(headerId);
// //       const result = isMandatory || !isExplicitlyUnchecked;
// //       console.log(`New mode - ${headerKey}: mandatory=${isMandatory}, unchecked=${isExplicitlyUnchecked}, result=${result}`);
// //       return result;
// //     }
// //   });
  
// //   console.log('Filtered headers count for Tab 6:', filteredHeaders.length);
// //   console.log('ACCESSORIES TOTAL in filtered:', filteredHeaders.find(p => p.header?.header_key === 'ACCESSORIES TOTAL'));
...
// //     // In edit mode, we should use the already set selected_accessories from booking data
// //     // The formData.selected_accessories is already set from fetchBookingDetails
// //     // We just need to update the accessories list
// //     setAccessories(filteredAccessories);
...
// //       })
// //       .filter((price) => {
// //         // EXCLUDE these specific summary/total headers from calculations
// //         const excludedHeaders = [
// //           'ON ROAD PRICE (A)',
// //           'TOTAL ONROAD + ADDON SERVICES',
// //           'TOTAL ONROAD+ADDON SERVICES',
// //           'ADDON SERVICES TOTAL (B)',
// //           'ACCESSORIES TOTAL', // EXCLUDE from calculations
// //           'ON ROAD PRICE',
// //           'ADDON SERVICES TOTAL',
// //           'ADD ON SERVICES TOTAL',
// //           'TOTAL AMOUNT',
...
// //     // ALWAYS include ACCESSORIES TOTAL header if it exists in the model
// //     const accessoriesTotalHeader = getSelectedModelHeaders().find(
// //       price => price.header && price.header.header_key === 'ACCESSORIES TOTAL'
// //     );
// //     if (accessoriesTotalHeader && accessoriesTotalHeader.header && accessoriesTotalHeader.header._id) {
// //       const accessoriesTotalHeaderId = accessoriesTotalHeader.header._id;
// //       if (!headersToSubmit.includes(accessoriesTotalHeaderId)) {
// //         headersToSubmit.push(accessoriesTotalHeaderId);
// //       }
// //     }
...
// //     {(() => {
// //       // Get headers for Tab 6 with proper branch filtering
...
// //         console.log('DEBUG - All prices in model:', allPrices.length);
// //         allPrices.forEach((p, i) => {
// //           console.log(`Price ${i}:`, {
// //             header_key: p.header?.header_key,
...
// //             console.log('Including price with no branch_id:', price.header?.header_key);
// //             return true;
// //           }
          
// //           // If price has branch_id, check if it matches selected branch
// //           const matches = price.branch_id === formData.branch;
// //           console.log(`Checking ${price.header?.header_key}: branch_id=${price.branch_id}, formBranch=${formData.branch}, matches=${matches}`);
// //           return matches;
// //         });
        
// //         console.log('Tab 6 - All prices:', allPrices.length);
// //         console.log('Tab 6 - Filtered prices by branch:', filteredPrices.length);
// //         console.log('Tab 6 - ACCESSORIES TOTAL in filtered:', filteredPrices.find(p => p.header?.header_key === 'ACCESSORIES TOTAL'));
        
// //         // Now filter to show only headers that should be displayed
// //         return filteredPrices.filter((price) => {
// //           if (!price.header || !price.header._id) {
// //             console.log('Skipping price without header:', price);
// //             return false;
// //           }
...
// //           // ALWAYS show ACCESSORIES TOTAL header in Tab 6
// //           if (headerKey === 'ACCESSORIES TOTAL') {
// //             console.log('Including ACCESSORIES TOTAL');
// //             return true;
// //           }
...
// //       console.log('Tab 6 headers to display:', tab6Headers.length);
...
// //                       if (!shouldShowHeader) {
// //                         console.log('Excluding HPA header from display:', headerKey);
// //                         return;
// //                       }
...
// //                       const isExplicitlyUnchecked = formData.uncheckedHeaders && 
// //                         formData.uncheckedHeaders.includes(headerId);
// //                       // For ACCESSORIES TOTAL, always treat as checked/selected
// //                       const isChecked = isAccessoriesTotal ? true : (isMandatory || !isExplicitlyUnchecked);
...
// //                           console.log(`Adjusting ADDON SERVICES TOTAL (${headerKey}): ${finalPrice} - ${discountApplied} = ${adjustedPrice}`);
// //                         } else if (headerKey === 'ACCESSORIES TOTAL') {
// //                           // ACCESSORIES TOTAL - show as read-only, no discount adjustments
// //                           adjustedPrice = finalPrice;
// //                           console.log(`ACCESSORIES TOTAL (${headerKey}): ${finalPrice} (read-only)`);
// //                         }
// //                       }
...
// //                       // For ACCESSORIES TOTAL, use the header price directly without any calculations
// //                       let taxable, cgstAmount, sgstAmount, cgstRate, sgstRate, lineTotal;
                      
// //                       if (isAccessoriesTotal) {
// //                         // For ACCESSORIES TOTAL, just show the value without GST calculations
// //                         taxable = finalPrice;
// //                         cgstAmount = 0;
// //                         sgstAmount = 0;
// //                         cgstRate = 0;
...
// //                       } else if (isSpecialHeader) {
// //                         // For other special headers, the "discount" is already applied to the price
// //                         taxable = calculateTaxableAmount(adjustedPrice, 0, gstRate, formData.customer_type);
// //                         const gstCalculation = calculateGST(taxable, gstRate, formData.customer_type);
// //                         cgstAmount = gstCalculation.cgstAmount;
// //                         sgstAmount = gstCalculation.sgstAmount;
...
// //                                 checked={isChecked}
// //                                 onChange={(e) => {
// //                                   if (!isMandatory && !isAccessoriesTotal) {
// //                                     const isNowChecked = e.target.checked;
// //                                     handleHeaderSelection(headerId, isNowChecked);
// //                                   }
// //                                 }}
// //                                 disabled={isMandatory || isAccessoriesTotal} // Disable for ACCESSORIES TOTAL
// //                                 style={{ marginRight: '10px' }}
// //                               />
// //                               <span>
// //                                 {header.header_key} 
// //                                 {isMandatory ? '(Mandatory)' : '(Optional)'}
// //                                 {isAccessoriesTotal ? ' (Read Only)' : ''} 
// //                                 {headerKey.startsWith('HP') || headerKey.startsWith('HPA') ? ' (HPA-related)' : ''}
// //                               </span>
// //                             </div>
// //                           </CTableDataCell>
...
// //                           <CTableDataCell>₹{finalPrice.toFixed(2)}</CTableDataCell>
// //                           <CTableDataCell>
// //                             {/* For ACCESSORIES TOTAL, show as read-only without discount field */}
// //                             {isAccessoriesTotal ? (
// //                               <div style={{ 
// //                                 padding: '8px', 
// //                                 backgroundColor: '#f8f9fa', 
...
//                       if (isAccessoriesTotal) {
//                         // For ACCESSORIES TOTAL, use the adjusted price (after discount)
//                         taxable = calculateTaxableAmount(adjustedPrice, 0, gstRate, formData.customer_type);
//                         const gstCalculation = calculateGST(taxable, gstRate, formData.customer_type);
//                         cgstAmount = gstCalculation.cgstAmount;
//                         sgstAmount = gstCalculation.sgstAmount;
...
//                                   }
//                                 }}
//                                 disabled={isMandatory || isAccessoriesTotal}
//                                 style={{ marginRight: '10px' }}
//                               />
//                               <span>
//                                 {header.header_key} 
...
//                                 {discountApplied > 0 && (
//                                   <div style={{ fontSize: '11px', color: '#666', marginTop: '2px' }}>
//                                     Original: ₹{finalPrice.toFixed(2)} | Discount: -₹{discountApplied.toFixed(2)}
//                                   </div>
//                                 )}
//                               </div>
//                             ) : isSpecialHeader ? (
                  ...
                  <div className="model-headers-section" style={{ marginTop: '20px' }}>
                    ...
                    <div className="table-responsive">
                      <CTable striped hover responsive>
                        ...
                        <CTableBody>
                          {getSelectedHeadersForTab6().map((price) => {
                            const header = price.header;
                            const isMandatory = header.is_mandatory;
                            const isDiscountAllowed = header.is_discount;
                            ...
                            // Find matching accessories for this header
                            // Find matching accessories for this header - ONLY SELECTED ONES
const selectedMatchingAccessories = accessories.filter(accessory => 
  accessory.categoryDetails?.header_key === header.header_key &&
  (isEditMode ? 
    formData.selected_accessories.includes(accessory._id) : // In edit mode
...
const accessoryPrice = selectedMatchingAccessories.length > 0 
  ...
  : 0;
                            ...
                            const headerPrice = price.value || 0;
                            // Use whichever is higher: header price or accessory price
                            const finalPrice = Math.max(headerPrice, accessoryPrice);
                            ...
                            // Calculate taxable amount (with discount, but NOT with subsidy)
                            const taxable = calculateTaxableAmount(finalPrice, discountAmount, gstRate, formData.customer_type);
                            
                            // Calculate GST amounts
                            const gstCalculation = calculateGST(taxable, gstRate, formData.customer_type);
                            const { cgstAmount, sgstAmount, cgstRate, sgstRate } = gstCalculation;
                            
                            // Calculate line total (with discount but NOT subsidy)
                            ...
                            return (
                              <CTableRow key={headerId}>
                                ...
                                <CTableDataCell>₹{finalPrice.toFixed(2)}</CTableDataCell>
                                <CTableDataCell>
                                  {/* The discount field should show ONLY actual discounts, NOT subsidy */}
                                  <CFormInput
                                    type="number"
                                    min="0"
                                    step="0.01"
