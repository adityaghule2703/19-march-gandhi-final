...
import CIcon from '@coreui/icons-react';
import { cilPencil } from '@coreui/icons';
import { showSuccess } from '../../utils/sweetAlerts'; // Import showSuccess
...
function RCConfirmation() {
  ...
  // Tab-level CREATE permission for COMPLETED RC tab (for OK button)
  const canCreateInCompletedRCTab = hasSafePagePermission(
    permissions,
    MODULES.RTO,
    PAGES.RTO.RC_CONFIRMATION,
  ...
  const handleApproveRC = async (rtoId) => {
    // Check CREATE permission for the COMPLETED RC tab
    if (!canCreateInCompletedRCTab) {
      showError('You do not have permission to approve RC');
      return;
    }
    
    try {
      setActionLoadingId(rtoId);
      const response = await axiosInstance.post(`/rtoProcess/approve/${rtoId}/rc`);
      
      if (response.data.success) {
        showSuccess('RC approved successfully!');
        // Refresh the data after successful approval
        await fetchLocationData();
      }
    } catch (error) {
      const errorMessage = error.response?.data?.message || error.message || 'Failed to approve RC';
            ...
            {filteredApproved.length === 0 ? (
              ...
              filteredApproved.map((item, index) => (
                <CTableRow key={item._id || index}>
                  ...
                  <CTableDataCell>
                    {item.displayStatus?.rc === 'Verified' ? (
                      ...
                      canCreateInCompletedRCTab ? (
                        <CButton 
                          size="sm" 
                          color="success"
                          onClick={() => handleApproveRC(item._id)}
