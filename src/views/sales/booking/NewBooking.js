...
function BookingForm() {
  ...
  // State for global discount distribution
  ...
  const transformPriceData = (price) => {
    // If price has nested header object (from models list API)
    ...
    // Otherwise, use the root level properties (from single model API)
    return {
      ...price,
      header: {
        _id: price.header_id,
 ...
 // ===== Function to distribute discount based on priority and percentages =====
const calculateDiscountDistribution = (totalDiscountAmount, headersList) => {
  console.log('=== calculateDiscountDistribution called ===');
  console.log('Total Discount Amount:', totalDiscountAmount);
  console.log('Headers List length:', headersList.length);
  
  if (!headersList || headersList.length === 0) {
    console.log('No headers list provided');
    return {};
  }
  
  // Filter headers that have discount_percentage and are discountable
  const discountableHeaders = headersList
    .filter(price => {
      const hasDiscountPercentage = price.discount_percentage !== undefined && price.discount_percentage !== null && price.discount_percentage !== '';
      const isDiscountable = price.is_discount === true;
      
      if (hasDiscountPercentage && isDiscountable) {
        console.log(`✅ Discountable: ${price.header_key}, priority=${price.discount_priority}, percentage=${price.discount_percentage}%`);
        return true;
      }
      return false;
    })
  ...
  console.log('Discountable Headers found:', discountableHeaders.length);
  
  if (discountableHeaders.length === 0) {
    console.log('No discountable headers found!');
    return {};
  }
  
  if (!totalDiscountAmount || totalDiscountAmount <= 0) {
    console.log('Total discount amount is 0 or invalid');
    return {};
  }
  ...
  console.log(`\n=== Distributing ₹${remainingDiscount} sequentially by priority ===\n`);
  
  for (const price of discountableHeaders) {
    const percentage = parseFloat(price.discount_percentage);
    const headerId = price.header_id;
    const priority = price.discount_priority;
    
    if (remainingDiscount <= 0) {
      distribution[headerId] = 0;
      console.log(`Priority ${priority} - ${price.header_key}: ${percentage}% = ₹0 (no discount remaining)`);
      continue;
    }
    
    // Calculate discount for this header as percentage of the ORIGINAL total discount amount
    let discountForHeader = totalDiscountAmount * (percentage / 100);
    
    // If calculated discount is more than remaining discount, give whatever is left
    if (discountForHeader > remainingDiscount) {
      discountForHeader = remainingDiscount;
    }
    
    distribution[headerId] = discountForHeader;
    remainingDiscount -= discountForHeader;
    
    console.log(`Priority ${priority} - ${price.header_key}: ${percentage}% of ₹${totalDiscountAmount} = ₹${discountForHeader.toFixed(2)}, Remaining: ₹${remainingDiscount.toFixed(2)}`);
  }
  
  console.log('\n=== Final Distribution ===');
  let totalDistributed = 0;
  Object.keys(distribution).forEach(headerId => {
    const header = headersList.find(h => h.header_id === headerId);
    const discountAmount = distribution[headerId];
    totalDistributed += discountAmount;
    const percentageOfTotal = (discountAmount / totalDiscountAmount) * 100;
    console.log(`${header?.header_key}: ₹${discountAmount.toFixed(2)} (${percentageOfTotal.toFixed(1)}% of entered ₹${totalDiscountAmount})`);
  });
  console.log(`Total discount distributed: ₹${totalDistributed.toFixed(2)} out of entered ₹${totalDiscountAmount}`);
  console.log(`Remaining unused discount: ₹${(totalDiscountAmount - totalDistributed).toFixed(2)}`);
  ...
  const handleGlobalDiscountChange = (value) => {
    console.log('=== handleGlobalDiscountChange called ===');
    console.log('New discount value:', value);
    ...
    if (discountAmount > 0) {
      ...
      console.log('Tab 6 Headers count:', tab6Headers.length);
      ...
      // Also update headerDiscounts for compatibility with existing calculations
      ...
      console.log('Clearing all discounts');
  ...
  const calculateDiscountUsageByCategory = (headers, discounts) => {
    ...
    headers.forEach((price) => {
      const categoryKey = price.category_key || '';
      const headerId = price.header_id;
      ...
      if (bookingData.model?.id) {
        ...
        const fetchModelDetails = async () => {
          try {
            ...
            // Transform prices to consistent format
            const transformedPrices = modelPrices.map(price => transformPriceData(price));
            
            // Get all headers using header_id
            const allHeaders = transformedPrices
            ...
            const mandatoryHeaders = transformedPrices
              .filter(price => price.is_mandatory === true)
              .map(price => price.header_id);
            ...
            setHeaderDiscounts(prev => {
              ...
              transformedPrices.forEach(price => {
                const headerId = price.header_id;
                if (headerId && updated[headerId] === undefined) {
                  updated[headerId] = 0;
                }
              });
  ...
  // Calculate total deal amount using discount distribution
  const calculateTotalDealAmount = () => {
    ...
    Object.values(discountDistribution).forEach(amount => {
    ...
    const exShowroomHeader = selectedHeaders.find(price => 
      price.header_key === 'Ex-SHOWROOM(INCLUDING 5% GST)'
    );
  ...
  const handleSubmit = async (e) => {
    ...
    console.log('Current discountDistribution:', discountDistribution);
    ...
    const headerDiscountsArray = Object.entries(discountDistribution)
      .filter(([headerId, discountAmount]) => {
        const isSelected = headersToSubmit.includes(headerId);
        const hasDiscount = discountAmount > 0;
        return isSelected && hasDiscount;
      })
      .map(([headerId, discountAmount]) => ({
        headerId,
              ...
              <>
                <div className="user-details">
                  <div className="input-box" style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '20px' }}>
                    ...
                    <div style={{ flex: '1', minWidth: '250px' }}>
                      <div className="details-container">
                        <span className="details">Global Discount Amount (₹)</span>
                      </div>
                      <CInputGroup>
                        <CInputGroupText className="input-icon">
                          <CIcon icon={cilMoney} />
                      ...
                      <small className="text-muted">
                        Discount will be distributed based on discount priority and percentage
                      </small>
                          ...
                          <CTable striped hover responsive>
                            ...
                            <CTableBody>
                              {tab6Headers.map((price) => {
                                ...
                                const gstRate = price.metadata?.gst_rate ? parseFloat(price.metadata.gst_rate) : 0;
                                const hsnCode = price.metadata?.hsn_code || 'N/A';
                                
                                const taxable = calculateTaxableAmount(finalPrice, discountAmount, gstRate, formData.customer_type);
                                const { cgstAmount, sgstAmount, cgstRate: cgstRateVal, sgstRate: sgstRateVal } = calculateGST(taxable, gstRate, formData.customer_type);
                                ...
                                if (headerKey === 'Ex-SHOWROOM(INCLUDING 5% GST)' && formData.subsidy_amount && isEVModel) {
                                  const subsidyAmountVal = parseFloat(formData.subsidy_amount) || 0;
                                  lineTotal = lineTotal - subsidyAmountVal;
                                }
                                ...
                                return (
                                  <CTableRow key={headerId}>
                                    <CTableDataCell>
                                      <div style={{ display: 'flex', alignItems: 'center' }}>
                                        <CFormCheck
                                          ...
                                          onChange={(e) => {
                                            if (!isMandatory) {
                                              handleHeaderSelection(headerId, e.target.checked);
                                            }
                                          }}
                                        ...
                                        <span>
                                          ...
                                          {isDiscountable && discountPriority !== 'N/A' ? ` (Discountable: Priority ${discountPriority})` : ' (Not Discountable)'}
                                        </span>
                                      </div>
                                    </CTableDataCell>
                                    ...
                                    <CTableDataCell>
                                      {isDiscountable ? (
                                        <strong style={{ color: discountAmount > 0 ? '#28a745' : '#6c757d' }}>
                                    ...
                                    <CTableDataCell>{cgstRateVal?.toFixed(2) || '0.00'}%</CTableDataCell>
                                    <CTableDataCell>₹{cgstAmount.toFixed(2)}</CTableDataCell>
                                    <CTableDataCell>{sgstRateVal?.toFixed(2) || '0.00'}%</CTableDataCell>
