...
// function StockTransfer() {
//   const [formData, setFormData] = useState({
//     fromType: 'branch', // New field: branch or subdealer
//     fromBranch: '',
//     fromSubdealer: '',
//     toType: 'branch',
//     toBranch: '',
...
//   const [branches, setBranches] = useState([]);
//   const [subdealers, setSubdealers] = useState([]);
//   const [fromSubdealers, setFromSubdealers] = useState([]); // Subdealers for from selection
//   const [vehicles, setVehicles] = useState([]);
//   const [filteredVehicles, setFilteredVehicles] = useState([]);
//   const [selectedVehicles, setSelectedVehicles] = useState([]);
//   const [isSubmitting, setIsSubmitting] = useState(false);
...
//     fetchBranches();
//     fetchAllSubdealers(); // Fetch all subdealers initially
//   }, []);

//   // Fetch subdealers for from selection when fromType is subdealer
//   useEffect(() => {
//     if (!canViewStockTransfer) return;
    
//     if (formData.fromType === 'subdealer') {
//       fetchFromSubdealers();
//     }
//   }, [formData.fromType]);

//   // Fetch to subdealers when toType is subdealer and from selection is valid
//   useEffect(() => {
//     if (!canViewStockTransfer) return;
    
//     if (formData.toType === 'subdealer') {
//       fetchToSubdealers();
//     }
//   }, [formData.toType, formData.fromBranch, formData.fromSubdealer]);

//   // Fetch OTP users based on from selection
//   useEffect(() => {
//     if (!canViewStockTransfer) return;
    
//     if (formData.fromType === 'branch' && formData.fromBranch) {
//       fetchOtpUsersForBranch(formData.fromBranch);
//     } else if (formData.fromType === 'subdealer' && formData.fromSubdealer) {
//       fetchOtpUsersForSubdealer(formData.fromSubdealer);
//     } else {
//       setOtpUsers([]);
//       setShowOtpSection(false);
//       setSelectedOtpUser('');
...
//       setOtpData({ otp: '', otpMethod: 'SMS' });
//     }
//   }, [formData.fromBranch, formData.fromSubdealer, formData.fromType]);
...
//   const fetchAllSubdealers = async () => {
//     if (!canViewStockTransfer) return;
...
//   const fetchFromSubdealers = async () => {
//     if (!canViewStockTransfer) return;
    
//     try {
//       // Fetch all subdealers that can be from sources
//       const response = await axiosInstance.get('/subdealers');
//       setFromSubdealers(response.data.data?.subdealers || []);
//     } catch (error) {
//       const message = showError(error);
//       if (message) setError(message);
//     }
...
//   const fetchToSubdealers = async () => {
//     if (!canViewStockTransfer) return;
    
//     try {
//       // Determine which subdealers to show based on from selection
//       let url = '/subdealers';
//       if (formData.fromType === 'branch' && formData.fromBranch) {
//         url = `/subdealers/branch/${formData.fromBranch}`;
//       } else if (formData.fromType === 'subdealer' && formData.fromSubdealer) {
//         // You might want to filter subdealers based on fromSubdealer
//         // For now, fetch all
//         url = '/subdealers';
//       }
...
//   const fetchOtpUsersForSubdealer = async (subdealerId) => {
//     if (!canViewStockTransfer) return;
...
//       // Filter users who belong to the selected subdealer AND have isStockTransferOTP = true
//       // You might need to adjust this based on your user-subdealer relationship
//       const filteredUsers = allUsers.filter(user => 
//         user.subdealer === subdealerId && 
//         user.isStockTransferOTP === true
//       );
      
//       setOtpUsers(filteredUsers);
      
//       // If there are no OTP users for this subdealer, don't show OTP section
//       if (filteredUsers.length === 0) {
//         setShowOtpSection(false);
//         setOtpVerified(true); // No OTP required if no users are configured
//       } else {
...
//   const fetchVehiclesForSource = async () => {
//     if (!canViewStockTransfer) return;
    
//     try {
//       let url = '';
//       if (formData.fromType === 'branch' && formData.fromBranch) {
//         url = `/vehicles/branch/${formData.fromBranch}?locationType=branch`;
//       } else if (formData.fromType === 'subdealer' && formData.fromSubdealer) {
//         url = `/vehicles/subdealer/${formData.fromSubdealer}?locationType=subdealer`;
//       } else {
//         return;
//       }
...
//         fromBranch: '',
//         fromSubdealer: '',
//         toType: 'branch', // Reset to branch when from type changes
//         toBranch: '',
//         toSubdealer: ''
//       }));
//       setVehicles([]);
//       setFilteredVehicles([]);
//       setSelectedVehicles([]);
//     } else if (name === 'fromBranch' || name === 'fromSubdealer') {
//       setFormData(prevData => ({ 
//         ...prevData, 
//         [name]: value,
//         toBranch: '',
...
//     if (!formData.fromType) newErrors.fromType = 'From type is required';
    
//     if (formData.fromType === 'branch' && !formData.fromBranch) {
//       newErrors.fromBranch = 'From branch is required';
//     } else if (formData.fromType === 'subdealer' && !formData.fromSubdealer) {
//       newErrors.fromSubdealer = 'From subdealer is required';
//     }
    
//     // Validate to fields
//     if (!formData.toType) newErrors.toType = 'To type is required';
    
//     if (formData.toType === 'branch' && !formData.toBranch) {
//       newErrors.toBranch = 'To branch is required';
//     } else if (formData.toType === 'subdealer' && !formData.toSubdealer) {
//       newErrors.toSubdealer = 'To subdealer is required';
//     }
...
//     try {
//       // Build payload based on fromType and toType
//       const payload = {
...
//       // Add from fields
//       if (formData.fromType === 'branch') {
//         payload.fromBranch = formData.fromBranch;
//       } else {
//         payload.fromSubdealer = formData.fromSubdealer;
//       }
...
//       } else {
//         payload.toSubdealer = formData.toSubdealer;
//       }
...
//       const fromBranchData = formData.fromType === 'branch' 
//         ? branches.find((b) => b._id === formData.fromBranch)
//         : null;
      
//       const fromSubdealerData = formData.fromType === 'subdealer'
//         ? fromSubdealers.find((s) => s._id === formData.fromSubdealer)
//         : null;
...
//         fromType: formData.fromType,
//         fromBranch: fromBranchData,
//         fromSubdealer: fromSubdealerData,
//         toBranch: toBranchData,
//         toSubdealer: toSubdealerData,
//         toType: formData.toType,
//         vehicles: selectedVehicleData,
...
//       // Reset form but keep fromType and from selection
//       setFormData({ 
//         fromType: formData.fromType,
//         fromBranch: formData.fromType === 'branch' ? formData.fromBranch : '',
//         fromSubdealer: formData.fromType === 'subdealer' ? formData.fromSubdealer : '',
//         toType: 'branch',
//         toBranch: '', 
//         toSubdealer: '',
...
//   // Determine if source is selected
//   const isSourceSelected = (formData.fromType === 'branch' && formData.fromBranch) || 
//                           (formData.fromType === 'subdealer' && formData.fromSubdealer);
...
//                 <form onSubmit={handleSubmit}>
//                   <div className="user-details">
//                     {/* First Row: From Type and From Selection */}
//                     <div className="input-box">
//                       <div className="details-container">
//                         <span className="details">From Type</span>
//                         <span className="required">*</span>
//                       </div>
//                       <CInputGroup>
//                         <CInputGroupText className="input-icon">
...
//                         </CFormSelect>
//                       </CInputGroup>
//                       {errors.fromType && <div className="invalid-feedback">{errors.fromType}</div>}
//                     </div>

//                     {/* From Branch Selection */}
//                     {formData.fromType === 'branch' && (
//                       <div className="input-box">
//                         <div className="details-container">
//                           <span className="details">From Branch</span>
//                           <span className="required">*</span>
...
//                     {/* From Subdealer Selection */}
//                     {formData.fromType === 'subdealer' && (
//                       <div className="input-box">
//                         <div className="details-container">
//                           <span className="details">From Subdealer</span>
//                           <span className="required">*</span>
//                         </div>
//                         <CInputGroup>
//                           <CInputGroupText className="input-icon">
...
//                           <CFormSelect 
//                             name="fromSubdealer" 
//                             value={formData.fromSubdealer} 
//                             onChange={handleChange} 
//                             invalid={!!errors.fromSubdealer}
//                             disabled={isSubmitting || !canCreateStockTransfer}
//                           >
//                             <option value="">-Select-</option>
//                             {fromSubdealers.map((subdealer) => (
//                               <option key={subdealer._id} value={subdealer._id}>
//                                 {subdealer.name}
//                               </option>
//                             ))}
//                           </CFormSelect>
//                         </CInputGroup>
//                         {errors.fromSubdealer && <div className="invalid-feedback">{errors.fromSubdealer}</div>}
//                       </div>
//                     )}
...
//                     {/* Second Row: To Type - Conditional based on From Type */}
//                     <div className="input-box">
//                       <div className="details-container">
//                         <span className="details">To Type</span>
//                         <span className="required">*</span>
//                       </div>
//                       <CInputGroup>
//                         <CInputGroupText className="input-icon">
...
//                           onChange={handleChange} 
//                           invalid={!!errors.toType}
//                           disabled={!isSourceSelected || isSubmitting || !canCreateStockTransfer}
//                         >
//                           {formData.fromType === 'branch' ? (
//                             // When From Type is Branch, show both options
//                             <>
//                               <option value="branch">Branch</option>
//                               <option value="subdealer">Subdealer</option>
//                             </>
//                           ) : (
//                             // When From Type is Subdealer, only show Branch option
//                             <>
//                               <option value="branch">Branch</option>
//                             </>
//                           )}
...
//                             {branches
//                               .filter(branch => {
//                                 // Don't show from branch in to branch options when from is branch
//                                 if (formData.fromType === 'branch') {
//                                   return branch._id !== formData.fromBranch;
//                                 }
//                                 return true;
//                               })
//                               .map((branch) => (
...
//                     {/* To Subdealer Selection - Only show when toType is subdealer */}
//                     {formData.toType === 'subdealer' && (
//                       <div className="input-box">
//                         <div className="details-container">
//                           <span className="details">To Subdealer</span>
...
//                             {subdealers
//                               .filter(subdealer => {
//                                 // Don't show from subdealer in to subdealer options when from is subdealer
//                                 if (formData.fromType === 'subdealer') {
//                                   return subdealer._id !== formData.fromSubdealer;
//                                 }
//                                 return true;
//                               })
//                               .map((subdealer) => (
...
//                 {vehicles.length > 0 && isSourceSelected ? (
//                   <div className="vehicle-table mt-4 p-3">
//                     <h5>In-Stock Vehicle Details ({vehicles.length} vehicles available)</h5>
...
//                                     onChange={(e) => handleVehicleSelect(vehicle._id, e.target.checked)}
//                                     checked={selectedVehicles.includes(vehicle._id)}
//                                     disabled={isSubmitting || !canCreateStockTransfer} // Removed OTP condition
//                                   />
//                                 </CTableDataCell>
//                               )}
//                               <CTableDataCell>{index + 1}</CTableDataCell>
//                               <CTableDataCell>
//   {vehicle.locationType === 'subdealer' && vehicle.subdealerLocation?.name 
...
//                     No in-stock vehicles found for the selected {formData.fromType}.
//                   </div>
//                 ) : null}
//               </div>
//             </div>
...
function StockTransfer() {
  ...
  const [branches, setBranches] = useState([]);
  const [subdealers, setSubdealers] = useState([]);
  const [fromSubdealers, setFromSubdealers] = useState([]);
  const [vehicles, setVehicles] = useState([]);
  const [filteredVehicles, setFilteredVehicles] = useState([]);
  const [selectedVehicles, setSelectedVehicles] = useState([]);
  const [isSubmitting, setIsSubmitting] = useState(false);
  ...
  const canCreateStockTransfer = canCreateInPage(permissions, MODULES.PURCHASE, PAGES.PURCHASE.STOCK_TRANSFER);
  const canDeleteStockTransfer = canDeleteInPage(permissions, MODULES.PURCHASE, PAGES.PURCHASE.STOCK_TRANSFER);
  const canExportReport = canCreateStockTransfer;
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      // Handle the exact error format you're receiving
      if (error.response && error.response.data) {
