...
const PAGE_SIZE_OPTIONS = [10, 25, 50, 100];
const DEFAULT_LIMIT = 10;
const LOW_QUANTITY_PAGE_SIZE_OPTIONS = [5, 10, 20, 50];
const LOW_QUANTITY_DEFAULT_LIMIT = 10;
...
const Parts = () => {
  ...
  const canViewParts = hasSafePagePermission(
    permissions, 
    MODULES.SERVICE_MANAGEMENT, 
    PAGES.SERVICE_MANAGEMENT.PARTS_LIST, 
    ACTIONS.VIEW
  );
  
  const canCreateParts = hasSafePagePermission(
    permissions, 
    MODULES.SERVICE_MANAGEMENT, 
    PAGES.SERVICE_MANAGEMENT.PARTS_LIST, 
    ACTIONS.CREATE
  ...
  const canUpdateParts = hasSafePagePermission(
    permissions, 
    MODULES.SERVICE_MANAGEMENT, 
    PAGES.SERVICE_MANAGEMENT.PARTS_LIST, 
    ACTIONS.UPDATE
  ...
  const canDeleteParts = hasSafePagePermission(
    permissions, 
    MODULES.SERVICE_MANAGEMENT, 
    PAGES.SERVICE_MANAGEMENT.PARTS_LIST, 
    ACTIONS.DELETE
  ...
  // Low Quantity Alert state
  const [lowQuantityModalVisible, setLowQuantityModalVisible] = useState(false);
  const [lowQuantityData, setLowQuantityData] = useState([]);
  const [lowQuantityCount, setLowQuantityCount] = useState(0);
  const [lowQuantityTotal, setLowQuantityTotal] = useState(0);
  const [lowQuantityThreshold, setLowQuantityThreshold] = useState(10);
  const [lowQuantityLoading, setLowQuantityLoading] = useState(false);
  const [lowQuantityError, setLowQuantityError] = useState(null);
  const [lowQuantityPagination, setLowQuantityPagination] = useState({
    page: 1,
    limit: LOW_QUANTITY_DEFAULT_LIMIT,
    totalPages: 1,
    totalItems: 0,
    hasNextPage: false,
    hasPrevPage: false
  ...
  // Form state - Updated to include quantity
  const [formData, setFormData] = useState({
    branchId: '',
    partNo: '',
    partName: '',
  ...
  // Fetch parts when branch, page, limit, or search changes
  useEffect(() => {
    if (selectedBranchId && canViewParts) {
      fetchParts();
    }
  }, [selectedBranchId, pagination.page, pagination.limit, canViewParts]);
  ...
  const handleAddClick = () => {
    if (!canCreateParts) {
      showError('You do not have permission to add parts');
      return;
    }
    const defaultBranchId = isSuperAdmin ? '' : selectedBranchId;
  ...
  // Updated handleEditClick to handle nested branchId object
  const handleEditClick = (part) => {
    if (!canUpdateParts) {
      showError('You do not have permission to edit parts');
      return;
    }
    setSelectedPart(part);
    
    // Handle branchId - it could be a string or an object
    let branchIdValue = '';
    if (part.branchId) {
      if (typeof part.branchId === 'string') {
        branchIdValue = part.branchId;
      } else if (typeof part.branchId === 'object' && part.branchId._id) {
        branchIdValue = part.branchId._id;
      }
    }
  ...
  const handleDeleteClick = (part) => {
    if (!canDeleteParts) {
      showError('You do not have permission to delete parts');
      return;
    }
    setSelectedPart(part);
    setDeleteModalVisible(true);
  ...
  const getBranchName = (branchId) => {
    ...
    // If branchId is an object with name property
    if (typeof branchId === 'object' && branchId.name) {
      return branchId.name;
    }
    
    // If branchId is a string, find it in branches array
  ...
  // Low Quantity Alert handlers
  const handleLowQuantityClick = () => {
    if (!canViewParts) {
      showError('You do not have permission to view low quantity alerts');
      return;
    }
    if (!selectedBranchId && isSuperAdmin) {
      showError('Please select a branch first');
    ...
    setLowQuantityModalVisible(true);
    // Reset pagination when opening modal
    setLowQuantityPagination(prev => ({ ...prev, page: 1 }));
    fetchLowQuantityParts(1, lowQuantityPagination.limit);
  };

  const fetchLowQuantityParts = async (page = 1, limit = lowQuantityPagination.limit) => {
    if (!selectedBranchId || !canViewParts) return;
    
    setLowQuantityLoading(true);
    setLowQuantityError(null);
    
    try {
      const params = new URLSearchParams();
      params.append('threshold', lowQuantityThreshold);
      params.append('branchId', selectedBranchId);
      params.append('page', page);
      params.append('limit', limit);
      
      const url = `/parts/low-quantity?${params.toString()}`;
      const response = await axiosInstance.get(url);
      
      if (response.data.success) {
        setLowQuantityData(response.data.data || []);
        setLowQuantityCount(response.data.count || 0);
        setLowQuantityTotal(response.data.total || 0);
        setLowQuantityThreshold(response.data.threshold || lowQuantityThreshold);
        
        // Update pagination
        if (response.data.pagination) {
          setLowQuantityPagination({
        ...
        setLowQuantityError(response.data.message || 'Failed to fetch low quantity parts');
      }
    } catch (error) {
      console.error('Error fetching low quantity parts:', error);
      setLowQuantityError(error.response?.data?.message || 'Failed to fetch low quantity parts');
      showError(error);
    } finally {
      setLowQuantityLoading(false);
    }
  };

  const handleLowQuantityPageChange = (newPage) => {
    if (newPage < 1 || newPage > lowQuantityPagination.totalPages) return;
    fetchLowQuantityParts(newPage, lowQuantityPagination.limit);
  };

  const handleLowQuantityLimitChange = (newLimit) => {
    setLowQuantityPagination(prev => ({ 
      ...prev, 
      limit: parseInt(newLimit, 10),
      page: 1
    }));
    fetchLowQuantityParts(1, parseInt(newLimit, 10));
  };

  const handleThresholdChange = (e) => {
    const value = parseInt(e.target.value, 10);
    if (!isNaN(value) && value > 0) {
      setLowQuantityThreshold(value);
    }
  };
  ...
  const resetLowQuantityModal = () => {
    setLowQuantityModalVisible(false);
    setLowQuantityData([]);
    setLowQuantityCount(0);
    setLowQuantityTotal(0);
    setLowQuantityThreshold(10);
    setLowQuantityLoading(false);
    setLowQuantityError(null);
    setLowQuantityPagination({
      page: 1,
  ...
  // Low Quantity Pagination render
  const renderLowQuantityPagination = () => {
    if (!lowQuantityPagination.totalItems || lowQuantityPagination.totalPages <= 1) return null;

    const startRecord = (lowQuantityPagination.page - 1) * lowQuantityPagination.limit + 1;
    const endRecord = Math.min(lowQuantityPagination.page * lowQuantityPagination.limit, lowQuantityPagination.totalItems);

    let startPage = Math.max(1, lowQuantityPagination.page - 2);
    let endPage = Math.min(lowQuantityPagination.totalPages, lowQuantityPagination.page + 2);
    if (lowQuantityPagination.page <= 3) endPage = Math.min(5, lowQuantityPagination.totalPages);
    if (lowQuantityPagination.page >= lowQuantityPagination.totalPages - 2) startPage = Math.max(1, lowQuantityPagination.totalPages - 4);
      ...
      <div className="mt-3 border-top pt-3">
        <div className="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-2">
          <div className="d-flex align-items-center gap-2">
            <CFormLabel className="mb-0 text-muted" style={{ fontSize: '13px' }}>Records per page:</CFormLabel>
            <CFormSelect
              value={lowQuantityPagination.limit}
              onChange={e => handleLowQuantityLimitChange(e.target.value)}
              style={{ width: '80px', height: '32px', fontSize: '13px' }}
              size="sm"
              disabled={lowQuantityLoading}
            >
              {LOW_QUANTITY_PAGE_SIZE_OPTIONS.map(n => <option key={n} value={n}>{n}</option>)}
            </CFormSelect>
          </div>
          <span className="text-muted" style={{ fontSize: '13px' }}>
            {lowQuantityLoading ? 'Loading…' : `Showing ${startRecord}–${endRecord} of ${lowQuantityPagination.totalItems} entries`}
          </span>
        </div>
        {lowQuantityPagination.totalPages > 1 && (
          <CPagination align="center" size="sm">
            <CPaginationItem 
              onClick={() => handleLowQuantityPageChange(1)} 
              disabled={lowQuantityPagination.page === 1 || lowQuantityLoading}
            >
              «
            </CPaginationItem>
            <CPaginationItem 
              onClick={() => handleLowQuantityPageChange(lowQuantityPagination.page - 1)} 
              disabled={!lowQuantityPagination.hasPrevPage || lowQuantityLoading}
            >
              <CIcon icon={cilChevronLeft} />
            </CPaginationItem>
            ...
            {displayedPages.map(p => (
              <CPaginationItem 
                key={p} 
                active={p === lowQuantityPagination.page} 
                onClick={() => handleLowQuantityPageChange(p)} 
                disabled={lowQuantityLoading}
              >
                {p}
              </CPaginationItem>
            ...
            {endPage < lowQuantityPagination.totalPages && (
              <>
                {endPage < lowQuantityPagination.totalPages - 1 && <CPaginationItem disabled>…</CPaginationItem>}
                <CPaginationItem 
                  onClick={() => handleLowQuantityPageChange(lowQuantityPagination.totalPages)} 
                  disabled={lowQuantityLoading}
                >
                  {lowQuantityPagination.totalPages}
                </CPaginationItem>
              </>
            )}
            ...
            <CPaginationItem 
              onClick={() => handleLowQuantityPageChange(lowQuantityPagination.page + 1)} 
              disabled={!lowQuantityPagination.hasNextPage || lowQuantityLoading}
            >
              <CIcon icon={cilChevronRight} />
            </CPaginationItem>
            <CPaginationItem 
              onClick={() => handleLowQuantityPageChange(lowQuantityPagination.totalPages)} 
              disabled={lowQuantityPagination.page === lowQuantityPagination.totalPages || lowQuantityLoading}
            >
              »
            </CPaginationItem>
          </CPagination>
  ...
  // Download Template handler - Only users with CREATE permission can download template
  const handleDownloadTemplate = async () => {
    ...
    try {
      const response = await axiosInstance.get('/parts/download-template', {
        responseType: 'blob',
        validateStatus: (status) => status < 500
      });
      ...
      const link = document.createElement('a');
      link.href = downloadUrl;
      link.setAttribute('download', 'parts_import_template.xlsx');
  ...
  const handleExportClick = () => {
    if (!canViewParts) {
      showError('You do not have permission to export parts');
      return;
    }
    if (!selectedBranchId && isSuperAdmin) {
      showError('Please select a branch first');
  ...
  // Import handlers - Only users with CREATE permission can import
  const handleImportClick = () => {
    if (!canCreateParts) {
      showError('You do not have permission to import parts');
      return;
    }
    if (!selectedBranchId && isSuperAdmin) {
      showError('Please select a branch first');
  ...
  const handleEditSubmit = async () => {
    if (!canUpdateParts) {
      showError('You do not have permission to update parts');
      return;
    }
    if (!validateForm()) return;
    if (!selectedPart) return;
  ...
  if (!canViewParts) {
    return (
      <div className="text-center py-5">
        ...
        <p className="text-muted">You don't have permission to view parts.</p>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <CCard className='table-container mt-4'>
        <CCardHeader className='card-header d-flex justify-content-between align-items-center'>
          <div>
            {/* Only show Add Part button if user has CREATE permission */}
            {canCreateParts && (
              <CButton size="sm" className="action-btn me-1" onClick={handleAddClick}>
                <CIcon icon={cilPlus} className='icon' /> Add Part
              </CButton>
            ...
            {/* Download Template - requires CREATE permission */}
            {canCreateParts && (
              <CButton size="sm" className="action-btn me-1" onClick={handleDownloadTemplate} disabled={templateLoading}>
                <CIcon icon={cilCloudDownload} className='icon' /> 
                {templateLoading ? 'Downloading...' : 'Download Template'}
              </CButton>
            )}
            
            {/* Import Excel - requires CREATE permission only */}
            {canCreateParts && (
              <CButton size="sm" className="action-btn me-1" onClick={handleImportClick}>
                <CIcon icon={cilCloudUpload} className='icon' /> Import Excel
              </CButton>
            ...
            {/* Export Excel - requires VIEW permission */}
            {canViewParts && (
              <CButton size="sm" className="action-btn me-1" onClick={handleExportClick}>
                <CIcon icon={cilPrint} className='icon' /> Export Excel
              </CButton>
            ...
            {/* Low Quantity Alert - requires VIEW permission */}
            {canViewParts && (
              <CButton size="sm" className="action-btn me-1" onClick={handleLowQuantityClick}>
                <CIcon icon={cilBell} className='icon' /> Low Quantity Alert
              </CButton>
            )}
          </div>
        </CCardHeader>
        <CCardBody>
                    ...
                    parts.map((part, index) => {
                      ...
                      return (
                        <CTableRow key={part._id}>
                          ...
                          <CTableDataCell>{part.hsnCode || '-'}</CTableDataCell>
                          <CTableDataCell>
                            <CBadge color={part.quantity > 0 ? 'success' : 'danger'}>
                              {part.quantity || 0}
                            </CBadge>
                          </CTableDataCell>
                          <CTableDataCell>{formatDate(part.createdAt)}</CTableDataCell>
          ...
          {/* Pagination */}
          {selectedBranchId && canViewParts && renderPagination()}
        </CCardBody>
      </CCard>

      {/* Add Part Modal - Only show if user has CREATE permission */}
      {canCreateParts && (
        <CModal size="lg" visible={addModalVisible} onClose={() => setAddModalVisible(false)} alignment="center">
          <CModalHeader>
            <CModalTitle>
          ...
          <CModalBody>
            ...
            <CRow className="mb-3">
              <CCol md={6}>
                ...
                <CFormInput
                  value={formData.partNo}
                  onChange={(e) => {
                    setFormData({ ...formData, partNo: e.target.value.toUpperCase() });
                    if (formErrors.partNo) {
                      setFormErrors({ ...formErrors, partNo: '' });
                    }
                    setApiError(null);
                  }}
                  placeholder="Enter part number"
                  className={formErrors.partNo ? 'is-invalid' : ''}
                />
                {formErrors.partNo && <small className="text-danger">{formErrors.partNo}</small>}
              </CCol>
              <CCol md={6}>
                ...
                <CFormInput
                  ...
                  onChange={(e) => {
                    ...
                    if (formErrors.partName) {
                      setFormErrors({ ...formErrors, partName: '' });
                    }
                    setApiError(null);
                  }}
                  placeholder="Enter part name"
                  className={formErrors.partName ? 'is-invalid' : ''}
                />
                {formErrors.partName && <small className="text-danger">{formErrors.partName}</small>}
              </CCol>
            </CRow>
            ...
            <CRow className="mb-3">
              ...
              <CCol md={6}>
                ...
                />
                {formErrors.quantity && <small className="text-danger">{formErrors.quantity}</small>}
              </CCol>
            </CRow>
      ...
      {/* Edit Part Modal - Only show if user has UPDATE permission */}
      {canUpdateParts && (
        <CModal size="lg" visible={editModalVisible} onClose={() => setEditModalVisible(false)} alignment="center">
          <CModalHeader>
            <CModalTitle>
      ...
      {/* Low Quantity Alert Modal */}
      {canViewParts && (
        <CModal visible={lowQuantityModalVisible} onClose={resetLowQuantityModal} alignment="center" size="lg">
          <CModalHeader>
            <CModalTitle>
          ...
          <CModalBody>
            {lowQuantityError && (
              <CAlert color="danger" className="mb-3">
                <div className="d-flex align-items-start">
                  ...
                  <div>
                    <strong>Error!</strong>
                    <p className="mb-0 mt-1">{lowQuantityError}</p>
                  </div>
                </div>
              </CAlert>
            )}
            ...
            <div className="mb-3">
              <div className="d-flex align-items-end gap-3">
                <div style={{ flex: 1 }}>
                  ...
                  />
                  <small className="text-muted">Parts with quantity less than or equal to this threshold will be shown</small>
                </div>
                <CButton 
                  color="primary" 
            ...
            {lowQuantityLoading ? (
              <div className="text-center py-4">
                ...
                <p className="mt-2 text-muted">Loading low quantity parts...</p>
              </div>
            ) : lowQuantityData.length > 0 ? (
              <>
                <div className="mb-3">
                  <CAlert color="warning">
                    <strong>Found {lowQuantityTotal} part(s)</strong> with quantity ≤ {lowQuantityThreshold}
                    {lowQuantityPagination.totalItems > 0 && lowQuantityPagination.totalItems !== lowQuantityTotal && (
                      <span> (Showing {lowQuantityPagination.totalItems})</span>
                    )}
                  </CAlert>
                </div>
                <div className="responsive-table-wrapper">
                  <CTable striped bordered hover className='responsive-table'>
                    ...
                    <CTableBody>
                      {lowQuantityData.map((part, index) => {
                        const globalIndex = (lowQuantityPagination.page - 1) * lowQuantityPagination.limit + index + 1;
                        return (
                          <CTableRow key={part._id}>
                            <CTableDataCell>{globalIndex}</CTableDataCell>
                            <CTableDataCell><strong>{part.partNo}</strong></CTableDataCell>
                ...
                {/* Low Quantity Pagination */}
                {renderLowQuantityPagination()}
              </>
            ) : (
              <div className="text-center py-4">
                <CIcon icon={cilCheckCircle} style={{ fontSize: '48px' }} className="text-success mb-3" />
                <h5 className="text-success">No low quantity parts found</h5>
                <p className="text-muted">All parts have quantity greater than the threshold of {lowQuantityThreshold}</p>
              </div>
            )}
          </CModalBody>
          <CModalFooter>
            <CButton color="secondary" onClick={resetLowQuantityModal}>
              Close
            </CButton>
          </CModalFooter>
        </CModal>
      ...
      {/* Import Excel Modal - Only show if user has CREATE permission */}
      {canCreateParts && (
        <CModal visible={importModalVisible} onClose={resetImportModal} alignment="center" size="lg">
          <CModalHeader>
            <CModalTitle>
