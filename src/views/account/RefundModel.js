...
const RefundModel = ({ show, onClose, bookingData, onRefundSaved }) => {
  const [formData, setFormData] = useState({
    bookingId: bookingData?._id || '',
    modeOfPayment: '',
    amount: '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess('Refund successfully recorded!');
      console.log('Refund response full:', response.data);
      ...
      // Method 4: Check allReceipts array for the latest receipt
      else if (response.data.data?.allReceipts?.length > 0) {
        // Get the most recent receipt (assuming it's the one just created)
      ...
      // Call onRefundSaved with message and receiptId before closing
      if (onRefundSaved) {
        onRefundSaved('Refund successfully recorded!', receiptId);
      }
      ...
      setError(err.response?.data?.error || 'Failed to process refund. Please try again.');
    } finally {
      setIsLoading(false);
    }
  };
