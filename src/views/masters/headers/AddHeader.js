...
import { CInputGroup, CInputGroupText, CFormInput, CFormSelect, CFormSwitch } from '@coreui/react';
import CIcon from '@coreui/icons-react';
import { cilCode, cilDollar, cilFile, cilKeyboard, cilListRich, cilStar, cilText, cilSortNumericUp } from '@coreui/icons';
import { useNavigate, useParams } from 'react-router-dom';
import { showFormSubmitError, showFormSubmitToast } from '../../../utils/sweetAlerts';
import FormButtons from '../../../utils/FormButtons';
import axiosInstance from '../../../axiosInstance';
...
function AddHeader() {
  ...
  const fetchHeader = async (id) => {
    try {
      ...
      setFormData({
        ...
        is_mandatory: header.is_mandatory ?? false,
        is_discount: header.is_discount ?? false,
        discount_priority: header.discount_priority || '',
  ...
  const handleSubmit = async (e) => {
    ...
    // Validation for discount fields (only if is_discount is true)
    if (formData.is_discount) {
      if (!formData.discount_priority) {
        formErrors.discount_priority = 'Discount priority is required when discount is enabled';
      }
      if (!formData.discount_percentage) {
        formErrors.discount_percentage = 'Discount percentage is required when discount is enabled';
      }
      
      // Validate percentage range
      if (formData.discount_percentage && (formData.discount_percentage < 0 || formData.discount_percentage > 100)) {
        formErrors.discount_percentage = 'Discount percentage must be between 0 and 100';
      }
      
      // Validate priority is positive number
      if (formData.discount_priority && formData.discount_priority < 1) {
        formErrors.discount_priority = 'Discount priority must be greater than 0';
      }
    }
    ...
    // Add discount fields to payload only if discount is enabled
    if (formData.is_discount) {
      payload.discount_priority = parseInt(formData.discount_priority);
        ...
        <div className="form-body">
          <form onSubmit={handleSubmit}>
            ...
            <div className="user-details">
              ...
              {/* NEW FIELDS - Only show when discount is enabled */}
              {formData.is_discount && (
                <>
                  <div className="input-box">
                    <div className="details-container">
                      <span className="details">Discount Priority</span>
                      <span className="required">*</span>
                    </div>
                    <CInputGroup>
                      <CInputGroupText className="input-icon">
                      ...
                      <CFormInput 
                        ...
                        placeholder="Enter discount priority (1, 2, 3...)"
                        className={errors.discount_priority ? 'is-invalid' : ''}
                      />
                    </CInputGroup>
                    {errors.discount_priority && <p className="error">{errors.discount_priority}</p>}
                  </div>
                  ...
                  <div className="input-box">
                    ...
                    <CInputGroup>
                      ...
                      <CFormInput 
                        ...
                        step="0.01"
                        className={errors.discount_percentage ? 'is-invalid' : ''}
                      />
                    </CInputGroup>
