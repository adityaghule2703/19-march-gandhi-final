...
const UpdateModel = () => {
  ...
  // Fetch headers when type changes
  useEffect(() => {
    if (formData.type) {
      fetchHeadersByType(formData.type);
    }
  }, [formData.type]);
  ...
  // New function to fetch headers by type
  const fetchHeadersByType = async (type) => {
    if (!type) return;
    
    setIsLoadingHeaders(true);
    try {
      const response = await axiosInstance.get(`/headers?type=${type}`);
      const headersData = response.data.data?.headers || [];
      setAvailableHeaders(headersData);
      
      // If we have existing prices, map them to the new headers
      if (formData.prices.length > 0) {
        // Keep only prices that match the new headers
        const headerIds = new Set(headersData.map(h => h._id));
        const filteredPrices = formData.prices.filter(price => 
          headerIds.has(price.header_id)
        );
        
        // Add any new headers that don't have prices
        const existingHeaderIds = new Set(filteredPrices.map(p => p.header_id));
        const newPrices = headersData
          .filter(header => !existingHeaderIds.has(header._id))
          .map(header => ({
        ...
        setFormData(prev => ({
          ...prev,
          prices: [...filteredPrices, ...newPrices]
        }));
      } else {
        // If no prices exist, create default prices for all headers
        const defaultPrices = headersData.map(header => ({
          header_id: header._id,
          header_key: header.header_key,
          value: 0,
          branch_id: branchId
      ...
      showError('Failed to load headers for the selected type');
    } finally {
  ...
  const fetchModelDetails = async () => {
    try {
      ...
      // Extract branch info from the first price
      const firstPrice = model.prices[0];
      if (firstPrice) {
        setBranchInfo({
          id: firstPrice.branch_id || branchId,
  ...
  const handlePriceChange = (headerId, newValue) => {
    ...
    // Clear error for this specific price
    if (priceErrors[headerId]) {
      setPriceErrors((prev) => {
        const newErrors = { ...prev };
        delete newErrors[headerId];
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="user-details">
              ...
              <div className="input-box">
                ...
                {errors.type && <p className="error">{errors.type}</p>}
                {isLoadingHeaders && <small className="text-muted">Loading headers...</small>}
              </div>
              ...
              {/* Render prices based on headers */}
              {isLoadingHeaders ? (
