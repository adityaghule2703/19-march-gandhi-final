...
function AddSubdealer() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Remove validation for coordinates (make them optional)
    // if (!formData.latLong.coordinates[0] || !formData.latLong.coordinates[1]) {
    //   formErrors['latLong.coordinates'] = 'Both longitude and latitude are required';
    // }
    ...
    try {
      // Handle coordinates - only include if both values are provided and valid
      const coordinates = [
        formData.latLong.coordinates[0] ? parseFloat(formData.latLong.coordinates[0]) : null,
        formData.latLong.coordinates[1] ? parseFloat(formData.latLong.coordinates[1]) : null
      ];

      // Only include coordinates in latLong if both are valid numbers
      const hasValidCoordinates = coordinates[0] !== null && coordinates[1] !== null && 
      ...
      if (hasValidCoordinates) {
        submitData.latLong.coordinates = coordinates;
      }

      // If no coordinates and no address, remove latLong entirely
      if (!hasValidCoordinates && !formData.latLong.address) {
        delete submitData.latLong;
      }
        ...
        <div className="form-body">
          <form onSubmit={handleSubmit}>
            <div className="user-details">
              ...
              {/* Second row: Longitude, Latitude, Address (3 fields) */}
              <div className="input-box">
                <div className="details-container">
                  <span className="details">Longitude</span>
              ...
              <div className="input-box">
                ...
                {/* Removed coordinates error display */}
              </div>
