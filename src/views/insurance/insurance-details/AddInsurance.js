...
const AddInsurance = ({ show, onClose, bookingData, insuranceData, onSuccess }) => {
  ...
  // Helper function to extract booking ID from various formats
  ...
  useEffect(() => {
    ...
    if (show) {
      ...
      if (isUpdate && insuranceData) {
        ...
        const bookingId = extractBookingId(insuranceData.booking);
        
        console.log('Update mode - Extracted booking ID:', bookingId);
        
        setFormData({
          booking: bookingId || '', // Now this will be a string ID
          insuranceProvider: insuranceData.insuranceProvider || '',
          policyNumber: insuranceData.policyNumber || '',
          rsaPolicyNumber: insuranceData.rsaPolicyNumber || '',
          cmsPolicyNumber: insuranceData.cmsPolicyNumber || '',
        ...
        // Clear any booking-related errors since we have the ID
        setErrors({});
      } else if (bookingData) {
        // For create mode, get booking ID from bookingData
        const bookingId = extractBookingId(bookingData);
        
        console.log('Create mode - Extracted booking ID:', bookingId);
        
        if (!bookingId) {
          setErrors({
            general: 'Booking ID is missing. Please select a booking first.'
          });
          return;
        }
        ...
        setErrors({
          general: 'No booking data available. Please try again.'
        });
      }
  ...
  // Get display values for customer name and chassis number
  const getCustomerName = () => {
    if (isUpdate && insuranceData?.booking) {
      return insuranceData.booking.customerName || '';
    }
    return bookingData?.customerDetails?.name || bookingData?.customerName || bookingData?.name || '';
  };

  const getChassisNumber = () => {
    if (isUpdate && insuranceData?.booking) {
      return insuranceData.booking.chassisNumber || '';
    }
    return bookingData?.chassisNumber || bookingData?.chassis_no || '';
  };
          ...
          <CForm>
            ...
            <CRow className="mb-3">
              ...
              <CCol md={6}>
                <label className="form-label">Document</label>
                <CFormInput type="file" name="document" onChange={handleFileChange} disabled={isLoading} />
                {isUpdate && insuranceData?.documents?.length > 0 && (
                  <small className="text-muted">Current file: {insuranceData.documents[0]?.name || insuranceData.documents[0]?.url?.split('/').pop()}</small>
                )}
              </CCol>
            </CRow>
          </CForm>
