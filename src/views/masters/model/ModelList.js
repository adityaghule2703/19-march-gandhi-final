...
const ModelList = () => {
  ...
  useEffect(() => {
    // Wait for branches to be loaded before setting initial filter
    if (canViewVehicles && userVerticleIds.length > 0 && branches.length > 0 && initialLoad) {
      // Set first branch as default if available
      if (branches.length > 0) {
        const defaultBranch = branches[0]._id;
        setSelectedBranch(defaultBranch);
        setTempSelectedBranch(defaultBranch);
        fetchData(defaultBranch, null, null, null);
        setInitialLoad(false);
      } else {
        // No branches available, fetch all data
        fetchData(null, null, null, null);
        setInitialLoad(false);
      }
    }
  }, [canViewVehicles, userVerticleIds, branches, initialLoad]);
  ...
  const handleImportSuccess = () => {
    ...
    if (selectedSubdealer) {
      fetchData(null, selectedSubdealer, selectedType, null);
    } else if (selectedBranch) {
      fetchData(selectedBranch, null, selectedType, null);
    } else {
      fetchData(null, null, selectedType, null);
    }
  };
