...
//   const [vehicles, setVehicles] = useState([]);
//   const [selectedVehicles, setSelectedVehicles] = useState([]);
//   const [selectedVehicleDetails, setSelectedVehicleDetails] = useState([]); // New state to store full vehicle objects
//   const [isSubmitting, setIsSubmitting] = useState(false);
//   const [searchTerm, setSearchTerm] = useState('');
//   const [isLoadingLocations, setIsLoadingLocations] = useState(false);
//   const [isLoadingVehicles, setIsLoadingVehicles] = useState(false);
...
//   // Permission checks using hasSafePagePermission directly
//   const canViewIDTList = hasSafePagePermission(
//     permissions, 
//     MODULES.STOCK_MOVEMENT, 
//     PAGES.STOCK_MOVEMENT.IDT_LIST, 
//     ACTIONS.VIEW
//   );
  
//   const canViewIDTHistory = hasSafePagePermission(
//     permissions, 
//     MODULES.STOCK_MOVEMENT, 
...
//   const canViewIDTRequests = hasSafePagePermission(
//     permissions, 
//     MODULES.STOCK_MOVEMENT, 
...
//   // For Transfer Vehicles - using CREATE permission on IDT List
//   const canTransferVehicles = hasSafePagePermission(
//     permissions, 
//     MODULES.STOCK_MOVEMENT, 
//     PAGES.STOCK_MOVEMENT.IDT_LIST, 
//     ACTIONS.CREATE
...
//   // Combined permission check for transfer action
//   const canPerformTransfer = isSuperAdmin || canTransferVehicles;

//   // Check if user can view vehicles (requires VIEW permission)
//   const canViewVehicles = isSuperAdmin || canViewIDTList;
...
//   // Fetch locations
//   const fetchLocations = async () => {
//     // Check if user has permission to view any IDT page or is superadmin
//     if (!isSuperAdmin && !canViewIDTList && !canViewIDTHistory && !canViewIDTRequests) {
//       console.warn('User does not have permission to view IDT page');
//       setError('You do not have permission to access Inter Dealer Transfer.');
//       return;
//     }
...
//   // Fetch vehicles with server-side pagination and search
//   const fetchVehicles = useCallback(async (page = 1, limit = DEFAULT_LIMIT, search = '') => {
//     // Check if user has permission to view IDT List or is superadmin
//     if (!isSuperAdmin && !canViewIDTList) {
//       setError('You do not have permission to view vehicles for transfer');
//       return;
//     }
...
//         // DO NOT filter selected vehicles - keep all selections
//         // Only clear selections when source location changes (handled elsewhere)
//         // This allows users to select vehicles across multiple pages
//       }
//     } catch (error) {
//       const message = showError(error);
//       if (message) setError(message);
...
//       setIsLoadingVehicles(false);
//     }
//   }, [formData.sourceDatabase, formData.sourceLocationType, formData.sourceLocationId, isSuperAdmin, canViewIDTList]);

//   // Fetch OTP users for location
//   const fetchOtpUsersForLocation = useCallback(async (locationType, locationId) => {
//     // OTP is only needed for transfers, so check transfer permission
//     if (!isSuperAdmin && !canTransferVehicles) {
//       setOtpUsers([]);
//       setShowOtpSection(false);
//       setOtpVerified(true);
//       return;
...
//         filteredUsers = allUsers.filter(user => 
//           user.branch === locationId && 
//           user.isStockTransferOTP === true
//         );
//       } else if (locationType === 'subdealer') {
//         filteredUsers = allUsers.filter(user => 
//           user.subdealer === locationId && 
//           user.isStockTransferOTP === true
//         );
//       }
...
//       setOtpVerified(true);
//     }
//   }, [isSuperAdmin, canTransferVehicles]);
...
//   // Trigger fetch when source location changes - only if user has view permission
//   useEffect(() => {
//     if (formData.sourceDatabase && formData.sourceLocationType && formData.sourceLocationId && canViewVehicles) {
//       // Clear selections when source location changes
//       setSelectedVehicles([]);
//       setSelectedVehicleDetails([]);
//       fetchVehicles(1, DEFAULT_LIMIT, searchTerm);
//     }
//   }, [formData.sourceDatabase, formData.sourceLocationType, formData.sourceLocationId, canViewVehicles]);

//   // Fetch OTP users when source location changes - only if user has transfer permission
//   useEffect(() => {
//     if (!isSuperAdmin && !canTransferVehicles) {
//       // If user can't transfer, skip OTP
//       return;
//     }
//     fetchOtpUsersForLocation(formData.sourceLocationType, formData.sourceLocationId);
//   }, [formData.sourceLocationType, formData.sourceLocationId, fetchOtpUsersForLocation, isSuperAdmin, canTransferVehicles]);
...
//   // Handle search with debounce - only if user has view permission
//   const handleSearch = useCallback((value) => {
...
//       fetchVehicles(1, vehiclePagination.limit, value);
//     }, 500);
//   }, [fetchVehicles, vehiclePagination.limit, canViewVehicles]);
...
//       showError('You do not have permission to transfer vehicles');
//       return;
//     }
...
//   const handleVehicleSelect = (vehicleId, isSelected) => {
//     // Find the vehicle object from current page
//     const vehicle = vehicles.find(v => v._id === vehicleId);
    
//     if (isSelected) {
//       // Add vehicle ID to selectedVehicles
//       setSelectedVehicles(prev => [...prev, vehicleId]);
//       // Store the full vehicle object if not already stored
//       if (vehicle && !selectedVehicleDetails.some(v => v._id === vehicleId)) {
//         setSelectedVehicleDetails(prev => [...prev, vehicle]);
//       }
//     } else {
//       // Remove vehicle ID
//       setSelectedVehicles(prev => prev.filter(id => id !== vehicleId));
//       // Remove vehicle object
//       setSelectedVehicleDetails(prev => prev.filter(v => v._id !== vehicleId));
//     }
//   };

//   const handleSelectAll = (e) => {
//     if (e.target.checked) {
//       // Add all vehicles from current page
//       const allVehicleIds = vehicles.map(vehicle => vehicle._id);
...
//       // Add all vehicle objects
//       setSelectedVehicleDetails(prev => {
//         const merged = [...prev];
//         vehicles.forEach(vehicle => {
//           if (!merged.some(v => v._id === vehicle._id)) {
//             merged.push(vehicle);
//           }
//         });
//         return merged;
//       });
//     } else {
//       // Remove all vehicles from current page
//       const allVehicleIds = new Set(vehicles.map(vehicle => vehicle._id));
//       setSelectedVehicles(prev => prev.filter(id => !allVehicleIds.has(id)));
//       setSelectedVehicleDetails(prev => prev.filter(v => !allVehicleIds.has(v._id)));
//     }
//   };
...
//     // Check transfer permission before submitting
//     if (!isSuperAdmin && !canPerformTransfer) {
//       showError('You do not have permission to transfer vehicles');
//       return;
//     }
...
//         // Use the stored vehicle details for the challan
//         const transferredVehicles = selectedVehicleDetails;
        
//         console.log('Selected Vehicles IDs:', selectedVehicles);
//         console.log('Transferred Vehicles:', transferredVehicles);
...
//   // If user doesn't have permission to view any IDT pages and is not superadmin, show access denied message
//   if (!isSuperAdmin && !canViewIDTList && !canViewIDTHistory && !canViewIDTRequests) {
//     return (
//       <div className="text-center py-5">
//         <CIcon icon={cilWarning} style={{ fontSize: '48px' }} className="text-warning mb-3" />
//         <h5 className="text-warning">Access Denied</h5>
//         <p className="text-muted">You don't have permission to access Inter Dealer Transfer.</p>
//       </div>
//     );
//   }
...
//                     {/* OTP User Selection Field - Only show if user has transfer permission */}
//                     {canPerformTransfer && showOtpSection && otpUsers.length > 0 && (
//                       <div className="input-box">
//                         <div className="details-container">
//                           <span className="details">OTP User</span>
//                           <span className="required">*</span>
...
//                     {/* OTP Input Field - Only show if user has transfer permission */}
//                     {canPerformTransfer && otpSent && !otpVerified && (
//                       <div className="input-box">
//                         <div className="details-container">
//                           <span className="details">Enter OTP</span>
//                           <span className="required">*</span>
...
//                     {/* OTP Verified Status - Only show if user has transfer permission */}
//                     {canPerformTransfer && otpVerified && (
//                       <div className="input-box">
//                         <div className="details-container">
//                           <span className="details">OTP Status</span>
//                         </div>
...
//                   <div className="form-footer">
//                     {/* Transfer Vehicles button - requires CREATE permission or superadmin */}
//                     <button 
//                       type="submit" 
//                       className="submit-button" 
//                       disabled={isSubmitting || !isSourceSelected || !isTargetSelected || (otpUsers.length > 0 && !otpVerified) || !canPerformTransfer}
//                       title={!canPerformTransfer ? "You do not have permission to transfer vehicles" : (otpUsers.length > 0 && !otpVerified ? "Please complete OTP verification first" : "")}
//                     >
//                       {isSubmitting ? (
//                         <>
//                           <CSpinner size="sm" className="me-2" />
...
//                 {/* Vehicles Table - only shown if user has view permission or is superadmin */}
//                 {isSourceSelected && canViewVehicles && (
//                   <div className="vehicle-table mt-4 p-3">
//                     <h5>In-Stock Vehicle Details ({vehiclePagination.total} vehicles available)</h5>
...
//                           label={`Select All (${vehicles.length})`}
//                           onChange={handleSelectAll}
//                           checked={vehicles.every(v => selectedVehicles.includes(v._id))}
//                           indeterminate={selectedVehicles.some(id => vehicles.some(v => v._id === id)) && !vehicles.every(v => selectedVehicles.includes(v._id))}
//                           className="d-inline-block"
//                         />
//                       )}
//                     </div>
...
//                     You do not have permission to view vehicles for transfer.
//                   </div>
//                 )}

//                 {isSourceSelected && vehiclePagination.total === 0 && !isLoadingVehicles && !searchTerm && canViewVehicles && (
//                   <div className="alert alert-info mt-4">
//                     No in-stock vehicles found at the selected source location.
//                   </div>
//                 )}
