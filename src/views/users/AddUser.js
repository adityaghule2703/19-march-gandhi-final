...
const paymentModeOptions = ['Cash', 'Bank', 'Finance Disbursement', 'Exchange', 'Pay Order'];
...
function AddUser() {
  ...
  const fetchUser = async (userId) => {
    try {
      ...
      setFormData({
        ...
        verticles: userVerticles,
        isStockTransferOTP: userIsStockTransferOTP,
        paymentModeAccess: userData.paymentModeAccess || [],
        discountLimits: userDiscountLimits,
        assignedSubdealers: userAssignedSubdealers
      });
  ...
  try {
    ...
    if (selectedOption && !formData.paymentModeAccess.includes(selectedOption)) {
      setFormData(prev => ({
        ...prev,
        paymentModeAccess: [...prev.paymentModeAccess, selectedOption]
      }));
    }
    setErrors(prev => ({ ...prev, paymentModeAccess: '' }));
  } catch (error) {
    const errorMessage = handleApiError(error, 'Handle Payment Mode Change');
    showApiErrorAlert(errorMessage, 'Payment Mode Selection Error');
  }
};

// Add a handler to remove payment mode
const removePaymentMode = (mode) => {
  try {
    setFormData(prev => ({
      ...prev,
      paymentModeAccess: prev.paymentModeAccess.filter(m => m !== mode)
    }));
  } catch (error) {
    const errorMessage = handleApiError(error, 'Remove Payment Mode');
    showApiErrorAlert(errorMessage, 'Payment Mode Removal Error');
  }
};

// Add helper function to get selected payment mode names (already have names)
const getSelectedPaymentModeNames = () => {
  return formData.paymentModeAccess;
};
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        verticles: formData.verticles,
        isStockTransferOTP: formData.isStockTransferOTP, // Added new field
         paymentModeAccess: formData.paymentModeAccess,
        ...(formData.discount !== '' && { discount: Number(formData.discount) }),
        ...(formData.type === 'employee' && formData.branch && { branch: formData.branch }),
        ...(formData.type === 'subdealer' && formData.subdealer && { subdealer: formData.subdealer }),
        ...(formData.totalDeviationAmount !== '' && { totalDeviationAmount: Number(formData.totalDeviationAmount) }),
            ...
            <FormErrorBoundary section="User Details">
              <div className="user-details">
                ...
                */}
                ...
                <div className="input-box">
  <div className="details-container">
    <span className="details">Payment Mode Access</span>
  </div>
  <CInputGroup>
    <CInputGroupText className="input-icon">
      <CIcon icon={cilDollar} />
    ...
    >
      ...
      {paymentModeOptions.map(mode => (
        <option 
          key={mode} 
          value={mode}
          disabled={formData.paymentModeAccess.includes(mode)}
        >
          {mode}
        </option>
      ))}
  ...
  <div className="mt-2">
    <div className="d-flex flex-wrap gap-2">
      {getSelectedPaymentModeNames().map((mode, index) => (
        <CBadge 
          key={`${mode}_${index}`} 
          color="primary"
          className="d-flex align-items-center"
    ...
    <small className="text-muted">
      {formData.paymentModeAccess.length} payment mode(s) selected (Optional - controls which payment modes the user can access)
    </small>
  </div>
</div>
