...
//   canCreateInPage
// } from '../../../utils/modulePermissions';
// import { cilPlus, cilPrint, cilCloudDownload, cilChevronLeft, cilChevronRight, cilSave } from '@coreui/icons';
// import QRCode from 'qrcode';
...
//   const [exportSubdealerId, setExportSubdealerId] = useState('all');
...
//       let url;
//       if (exportSubdealerId === 'all') {
//         url = `/reports/subdealer-allocations?subdealerId=all&startDate=${exportStartDate}&endDate=${exportEndDate}&format=excel`;
//       } else {
//         url = `/reports/subdealer-allocations?subdealerId=${exportSubdealerId}&startDate=${exportStartDate}&endDate=${exportEndDate}&format=excel`;
//       }
...
//       const fileName = `subdealer_allocations_${exportStartDate}_to_${exportEndDate}_${exportSubdealerId === 'all' ? 'all' : 'selected'}.xlsx`;
...
//             {/* "New Balance" button - not visible to subdealers */}
//             {canCreateOnAccount && !isSubdealer && (
//               <Link to="/subdealer-account/add-amount">
//                 <CButton size="sm" className="action-btn me-1">
//                   <CIcon icon={cilPlus} className='icon'/> New Balance
...
//               >
//                 <CIcon icon={cilSave} className='me-1'/> Export Excel
//               </CButton>
//             )}
//           </div>
//           <div className="text-muted">
...
//       {/* Export Modal */}
//       <CModal visible={showExportModal} onClose={() => setShowExportModal(false)} size="lg">
//         <CModalHeader>
//           <CModalTitle>Export Subdealer Allocations Report</CModalTitle>
//         </CModalHeader>
//         <CModalBody>
//           <div className="row">
//             <div className="col-md-12 mb-3">
//               <CFormLabel>Select Subdealer</CFormLabel>
//               <CFormSelect
//                 value={exportSubdealerId}
//                 onChange={(e) => setExportSubdealerId(e.target.value)}
//                 disabled={exporting}
//               >
//                 <option value="all">All Subdealers</option>
//                 {pagination.docs.map(subdealer => (
...
//   // Fetch all subdealers for export dropdown
//   const fetchAllSubdealersForExport = useCallback(async () => {
//     if (!canViewOnAccount) return;
...
//         setAllSubdealers(subdealers);
//       }
//     } catch (error) {
//       console.error('Error fetching all subdealers:', error);
//       showError('Failed to load subdealers for export');
//     } finally {
//       setLoadingSubdealers(false);
//     }
//   }, [canViewOnAccount, isSubdealer, userSubdealerId]);
...
//     fetchData(1, DEFAULT_LIMIT, '');
//     fetchAllSubdealersForExport(); // Fetch all subdealers for export dropdown
//   }, [canViewOnAccount]);
...
//                 value={exportSubdealerId}
//                 onChange={(e) => setExportSubdealerId(e.target.value)}
//                 disabled={exporting || loadingSubdealers}
//               >
//                 <option value="all">All Subdealers</option>
//                 {allSubdealers.map(subdealer => (
//                   <option key={subdealer._id} value={subdealer._id}>
//                     {subdealer.name}
//                   </option>
//                 ))}
//               </CFormSelect>
//               {loadingSubdealers && (
//                 <small className="text-muted">Loading subdealers...</small>
//               )}
//             </div>
...
const fetchReceiptsForSubdealer = useCallback(async (subdealerId) => {
  ...
  try {
    ...
    const response = await axiosInstance.get(`/subdealersonaccount/${subdealerId}/receipts`);
    ...
    if (response.data && response.data.success && response.data.data) {
      ...
      // Filter for CREDIT type entries (these are receipts/payments)
      // Also include entries where type is CREDIT or source is ON_ACCOUNT with CREDIT
      if (responseData.entries && Array.isArray(responseData.entries)) {
        receipts = responseData.entries.filter(entry => 
          entry.type === 'CREDIT' && 
          (entry.source === 'ON_ACCOUNT' || entry.source === 'PENALTY_PAYMENT_MODEL')
        );
      }
    } else if (response.data && response.data.docs) {
...
const printSubdealerReceipt = async (receiptId) => {
  try {
    ...
    // Extract branch information from API response
    const branch = receiptData.branch || {};
    const branchName = branch.name || 'GANDHI TVS NASHIK';
    ...
    const branchEmail = branch.email || 'gandhitvsnashik@gmail.com';
    const branchGstNumber = branch.gst_number || '27AACCG1234E1Z5';
    
    // Format full address
    const fullAddress = [branchAddress, branchCity, branchState, branchPincode]
...
Date: ${receivedDate}`;
    ...
    const receiptHTML = generateReceiptHTML(receiptData, qrCodeImage, receivedDate, currentDate, branch, branchName, fullAddress, branchPhone, branchEmail, branchGstNumber);
 ...
 // ============ GENERATE RECEIPT HTML ============
const generateReceiptHTML = (receiptData, qrCodeImage, receivedDate, currentDate, branch, branchName, fullAddress, branchPhone, branchEmail, branchGstNumber) => {
  const receiptNumber = receiptData.refNumber || receiptData._id || 'N/A';
  const subdealerName = receiptData.subdealer?.name || 'N/A';
  const amount = receiptData.amount || 0;
  const paymentMode = receiptData.paymentMode || 'Cash';
  ...
  const receivedByName = receiptData.receivedByName || receiptData.receivedBy?.name || 'System';
  const approvalStatus = receiptData.approvalStatus || 'Pending';
  const receiptBalance = receiptData.receiptBalance || amount;
...
<body>
  <div class="page">
    ...
    <div class="receipt-copy">
      <div class="header-container">
        <div class="header-left">
          ...
          <div class="dealer-info">
            ...
            Phone: ${branchPhone}<br>
            Email: ${branchEmail}<br>
            GSTIN: ${branchGstNumber}
          </div>
        </div>
        <div class="header-right">
          <div class="logo-qr-container">
      ...
      <div class="receipt-info">
        <div><strong>SUBDEALER PAYMENT RECEIPT</strong></div>
        <div><strong>Receipt Date:</strong> ${receivedDate}</div>
        <div><strong>Status:</strong> <span class="status-badge">${approvalStatus}</span></div>
      </div>
      ...
      <div class="payment-info-box">
        <div class="receipt-info">
          ...
          <div style="display: flex; justify-content: space-between; margin-top: 5px;">
            <span><strong>Receipt Balance:</strong></span>
            <span>₹${receiptBalance.toLocaleString('en-IN')}</span>
          </div>
          ${remark ? `<div style="margin-top: 5px;"><strong>Remark:</strong> ${remark}</div>` : ''}
        </div>
 ...
 const handleViewLedger = async (subdealer) => {
  try {
    // Use the new API endpoint for ledger/receipts
    const res = await axiosInstance.get(`/subdealersonaccount/${subdealer._id}/receipts`);
    ...
    if (res.data && res.data.success && res.data.data) {
      ...
      const allEntries = responseData.entries || [];
      
      // Separate credit entries (receipts/payments) for ledger view
      ledgerEntries = allEntries.filter(entry => 
        entry.type === 'CREDIT' && 
        (entry.source === 'ON_ACCOUNT' || entry.source === 'PENALTY_PAYMENT_MODEL')
      );
      
      // Get total on-account balance
      onAccountBalance = responseData.totals?.totalOnAccountBalance || 0;
    } else if (res.data && res.data.docs) {
      // Fallback for old structure
      ledgerEntries = res.data.docs || [];
              ...
              <tbody>
                ${ledgerEntries.length > 0 ? ledgerEntries
                  .map((entry) => {
                    ...
                    const receiptCredit = entry.credit || 0;
                    const receiptDate = entry.date || new Date(entry.timestamp).toLocaleDateString('en-GB');
                    const receiptRemark = entry.description || entry.remark || '';
                    const receiptRefNumber = entry.receiptNo || entry.id || '';
                    const receiptPaymentMode = entry.paymentMode || 'N/A';
                    ...
                    return `
                      <tr>
                        <td>${receiptDate}</td>
                        <td>
                          ${entry.source === 'ON_ACCOUNT' ? 'On-Account Payment' : 'Penalty Payment'}<br>
                          Mode: ${receiptPaymentMode}<br>
                          ${receiptRemark}
                        </td>
                        <td>${receiptRefNumber}</td>
