...
function AddBranch() {
  ...
  const fetchBranch = async (id) => {
    try {
      ...
      // Create full URLs for logo previews
      // Check if the API returns a complete URL or a relative path
      if (branchData.logo1) {
        if (branchData.logo1.startsWith('http')) {
          // It's already a complete URL
          setLogo1Preview(branchData.logo1);
        } else {
      ...
      if (branchData.logo2) {
        if (branchData.logo2.startsWith('http')) {
          // It's already a complete URL
          setLogo2Preview(branchData.logo2);
        } else {
          // It's a relative path, prepend base URL
          setLogo2Preview(`${axiosInstance.defaults.baseURL}${branchData.logo2}`);
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // For logo removal, we need to handle it differently
      // If logo was removed (preview cleared), we should send empty string
      if (id) {
        // Check if logo was removed
        if (logo1Preview === '' && originalData?.logo1) {
          form.append('logo1', '');
        }
        if (logo2Preview === '' && originalData?.logo2) {
          form.append('logo2', '');
        }
      }
