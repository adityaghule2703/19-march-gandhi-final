...
function AddAmount() {
  ...
  // Fetch user permissions and get payment mode access
  useEffect(() => {
    const fetchUserPaymentModes = async () => {
      setIsLoadingPaymentModes(true);
      try {
        const response = await axiosInstance.get('/auth/me');
        const paymentModes = response.data.data?.paymentModeAccessList || [];
        setAvailablePaymentModes(paymentModes);
      } catch (error) {
        console.error('Error fetching user payment modes:', error);
        // Fallback to default modes if API fails
        setAvailablePaymentModes(['Cash', 'Bank', 'UPI', 'RTGS', 'Cheque', 'Pay Order', 'Other']);
      } finally {
        setIsLoadingPaymentModes(false);
      }
    };

    fetchUserPaymentModes();
  }, []);
          ...
          <form onSubmit={handleSubmit}>
            <div className="user-details">
              ...
              <div className="input-box">
                ...
                {isLoadingPaymentModes && <p className="text-muted">Loading payment modes...</p>}
                {errors.paymentMode && <p className="error">{errors.paymentMode}</p>}
              </div>
