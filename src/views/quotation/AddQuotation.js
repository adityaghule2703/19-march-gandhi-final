...
// import { CInputGroup, CInputGroupText, CFormInput, CFormSwitch, CFormCheck, CSpinner } from '@coreui/react';
// import CIcon from '@coreui/icons-react';
// import { cilCalendar, cilLocationPin, cilMap, cilPhone, cilUser, cilSearch } from '@coreui/icons';
// import { useNavigate, useParams } from 'react-router-dom';
// import { showFormSubmitError, showFormSubmitToast } from '../../utils/sweetAlerts';
// import axiosInstance from '../../axiosInstance';
// import './customer.css';
...
//                         borderRadius: '4px'
//                       }}>
//                         <strong>Selected Models ({selectedModels.length}/2):</strong>{' '}
//                         {selectedModels.map((selected, index) => {
//                           const model = models.find(m => m._id === selected.model_id);
//                           return model ? (
//                             <span key={selected.model_id}>
//                               {model.model_name}
...
function AddCustomer() {
  ...
  const handleChange = (e) => {
    ...
    // Check for past date (only for expected_delivery_date)
    if (name === 'expected_delivery_date' && value) {
      const selectedDate = new Date(value);
      const today = new Date();
      today.setHours(0, 0, 0, 0);
  ...
  const validateTab2 = () => {
    ...
    if (!formData.mobile2) formErrors.mobile2 = 'This field is required';
    if (!formData.expected_delivery_date) {
      formErrors.expected_delivery_date = 'This field is required';
    } else {
