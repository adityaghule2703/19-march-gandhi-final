...
//       const response = await axiosInstance.get(`/subdealers/${selectedSubdealer}/financial-summary`);
//       setSubdealerData(response.data.data);
//       // The recentTransactions in the new API response contains the bookings data
//       setBookings(response.data.data.recentTransactions || []);
//     } catch (error) {
//       console.error('Error fetching subdealer financial summary:', error);
//       setError('Failed to load financial data for this subdealer');
...
//       default:
//         return <span className="badge bg-secondary">{paymentType || '-'}</span>;
//     }
//   };
...
//                 <CCard className="mb-4">
//                   <CCardHeader>
//                     <h6>Subdealer Summary: {subdealerData.subdealer?.name || 'N/A'}</h6>
//                   </CCardHeader>
//                   <CCardBody>
//                     <CRow>
//                       <CCol md={3}>
//                         <strong>Total Bookings:</strong> {subdealerData.bookingSummary?.totalBookings || 0}
//                       </CCol>
//                       <CCol md={3}>
//                         <strong>Total Amount:</strong> ₹{(subdealerData.bookingSummary?.totalBookingAmount || 0).toLocaleString()}
//                       </CCol>
//                       <CCol md={3}>
//                         <strong>Received:</strong> ₹{(subdealerData.bookingSummary?.totalReceivedAmount || 0).toLocaleString()}
//                       </CCol>
//                       <CCol md={3}>
//                         <strong>Remaining:</strong> ₹{(subdealerData.financialOverview?.totalOutstanding || 0).toLocaleString()}
//                       </CCol>
//                     </CRow>
//                     {receipts.length > 0 && (
//                       <CRow className="mt-3">
...
//                               </CButton>
//                             </CInputGroup>
//                             <small className="text-muted">Showing only bookings with pending balance</small>
//                           </CCol>
//                         </CRow>
//                       )}
...
//                                 <CTableDataCell>{getPaymentTypeBadge(booking.paymentType)}</CTableDataCell>
//                                 <CTableDataCell>₹{(booking.totalAmount || 0).toLocaleString()}</CTableDataCell>
//                                 <CTableDataCell>₹{(booking.receivedAmount || 0).toLocaleString()}</CTableDataCell>
//                                 <CTableDataCell>₹{(booking.balanceAmount || 0).toLocaleString()}</CTableDataCell>
...
//                           <p>
//                             {searchTerm
//                               ? `No bookings found with pending balance matching "${searchTerm}"`
//                               : 'No bookings with pending balance found for this subdealer.'}
//                           </p>
//                         </div>
//                       )}
//                     </>
...
//                               const balance = calculateAccessoryBalance(accessory);
//                               return (
//                                 <CTableRow key={accessory._id || index}>
//                                   <CTableDataCell>{index + 1}</CTableDataCell>
//                                   <CTableDataCell>{accessory._id?.substring(accessory._id.length - 6) || '-'}</CTableDataCell>
//                                   <CTableDataCell>{accessory.payment_type || '-'}</CTableDataCell>
//                                   <CTableDataCell>₹{(accessory.total_amount || 0).toLocaleString()}</CTableDataCell>
//                                   <CTableDataCell>₹{(accessory.net_amount || 0).toLocaleString()}</CTableDataCell>
//                                   <CTableDataCell>{formatDate(accessory.createdAt)}</CTableDataCell>
//                                   <CTableDataCell>{accessory.remark || '-'}</CTableDataCell>
//                                   <CTableDataCell>
//                                     <CButton
//                                       color="primary"
...
//                                 const balance = calculatePenaltyBalance(penalty);
//                                 return (
//                                   <CTableRow key={penalty._id || index}>
//                                     <CTableDataCell>{(penaltyPage - 1) * penaltyLimit + index + 1}</CTableDataCell>
//                                     <CTableDataCell>{penalty.referenceNumber || '-'}</CTableDataCell>
//                                     <CTableDataCell>₹{(penalty.amount || 0).toLocaleString()}</CTableDataCell>
//                                     <CTableDataCell>₹{(penalty.paidAmount || 0).toLocaleString()}</CTableDataCell>
//                                     <CTableDataCell>₹{balance.toLocaleString()}</CTableDataCell>
//                                     <CTableDataCell>{penalty.reason || '-'}</CTableDataCell>
//                                     <CTableDataCell>
//                                       <span className={`badge ${penalty.status === 'ACTIVE' ? 'bg-warning' : 'bg-success'}`}>
...
//                 <div className="mb-3">
//                   <CFormLabel>Customer Name</CFormLabel>
//                   <CFormInput type="text" value={selectedBooking?.customerName || ''} disabled readOnly />
//                 </div>
//               </CCol>
//               <CCol md={6}>
//                 <div className="mb-3">
...
//               <CFormInput type="text" value={selectedBooking?.paymentType || '-'} disabled readOnly />
//             </div>
...
//               <CFormInput 
//                 type="text" 
//                 value={selectedAccessory ? `₹${(selectedAccessory.net_amount || 0).toLocaleString()}` : ''} 
//                 disabled 
//                 readOnly 
//               />
//             </div>
...
function SubdealerCustomerManagement() {
 ...
 const fetchSubdealerReceipts = async () => {
  try {
    // Using the new financial-summary API
    const response = await axiosInstance.get(`/subdealers/${selectedSubdealer}/financial-summary`);
    
    console.log('Financial Summary API Response:', response.data);
    
    if (response.data && response.data.data && response.data.data.recentReceipts) {
      const recentReceipts = response.data.data.recentReceipts;
      
      // Filter receipts where remainingBalance > 0
      const receiptEntries = recentReceipts
        .filter(receipt => receipt.remainingBalance > 0)
        .map(receipt => {
          return {
            _id: receipt._id,
            refNumber: receipt.refNumber || `REC-${receipt._id?.substring(receipt._id?.length - 6) || ''}`,
            amount: receipt.amount || 0,
            allocatedTotal: receipt.allocatedTotal || 0,
            receiptRemaining: receipt.remainingBalance || 0,
            date: receipt.receivedDate || '',
            remark: receipt.remark || '',
            paymentMode: receipt.paymentMode || 'Bank',
            status: receipt.status || '',
            allocations: receipt.allocations || [],
            allocationCount: receipt.allocationCount || 0
          };
        });
      
      console.log('Processed receipts data (remainingBalance > 0):', receiptEntries);
      setReceipts(receiptEntries);
    } else {
      setReceipts([]);
    }
