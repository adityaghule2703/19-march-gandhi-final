...
//     <p>
//       <span className="location-label">Load Location:</span>
//       {fromBranch?.name || fromSubdealer?.name || 'N/A'}
//     </p>
//   </div>
//   <div className="location-item">
//     <p>
...
const TransferChallan = ({ 
  ...
  // Helper function to get load location
  const getLoadLocation = () => {
    if (fromBranch) return fromBranch.name;
    if (fromSubdealer) return fromSubdealer.name;
    return 'N/A';
  };
  ...
  const generateTransferChallanHTML = () => {
    const currentDate = formatDate(transferDetails?.createdAt || new Date());
    const destination = getDestination();
    const loadLocation = getLoadLocation();
