...
function HelmetInvoice() {
  ...
  // Permission checks for Helmet Invoice page under Sales module
  const canViewHelmetInvoice = canViewPage(permissions, MODULES.SALES, PAGES.SALES.HELMET_INVOICE);
  
  // For printing, we need CREATE permission
  const canCreateHelmetInvoice = hasSafePagePermission(
    permissions, 
    MODULES.SALES, 
    PAGES.SALES.HELMET_INVOICE, 
    ACTIONS.CREATE
  );
