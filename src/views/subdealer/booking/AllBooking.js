...
const AllBooking = () => {
  ...
  // Check if user is from RAJ AUTO LASALGAON
  const isRajAutoSubdealer = authUser?.subdealer?.name === 'RAJ AUTO LASALGAON';
  
  // Check if user is SUPERADMIN
  const isSuperAdmin = authUser?.roles?.some(role => 
    role.name === 'SUPERADMIN' || role.name === 'Superadmin' || role.name === 'superadmin'
  );
            ...
            <CTableBody>
              {currentRecords.length === 0 ? (
                ...
                currentRecords.map((booking, index) => {
                  ...
                  // Check if booking has any discounts
                  const hasDiscount = booking.discounts && booking.discounts.length > 0;
                  
                  // Determine if user can see Allocate Chassis button for this booking
                  // SUPERADMIN and RAJ AUTO LASALGAON always see it
                  // Other users see it only if NO discount is applied
                  const canSeeAllocateChassisForBooking = 
                    isSuperAdmin || 
                    isRajAutoSubdealer || 
                    !hasDiscount;
                    ...
                    <CTableRow key={index}>
                      ...
                      <CTableDataCell>
                        ...
                        >
                          ...
                          {/* MODIFIED: Show Allocate Chassis based on conditions */}
                          {tabIndex === 0 && canAllocateChassisInApprovedTab && canSeeAllocateChassisForBooking && (
                            <MenuItem onClick={() => handleAllocateChassis(bookingId)} style={{ color: 'black' }}>
                              <CIcon icon={cilPencil} className="me-2" /> Allocate Chassis
                            </MenuItem>
                          )}
