...
//                       Subdealers can only view commission reports for their own account
//                     </div>
//                   </div>
//                 ) : (
//                   <CFormSelect
...
//                   </CFormSelect>
//                 )}
//                 {!selectedSubdealer && !isSubdealer && <div className="form-text text-warning">Please select a subdealer to continue</div>}
//               </div>
...
const CalculateCommission = () => {
  ...
  // Page-level permission checks for Calculate Commission page under Subdealer Master module
  const canViewCommissionReport = canViewPage(permissions, MODULES.SUBDEALER_MASTER, PAGES.SUBDEALER_MASTER.CALCULATE_COMMISSION);
  const canCreateCommissionReport = canCreateInPage(permissions, MODULES.SUBDEALER_MASTER, PAGES.SUBDEALER_MASTER.CALCULATE_COMMISSION);

  useEffect(() => {
    if (!canViewCommissionReport) {
      showError('You do not have permission to view Commission Report');
      return;
    }
  ...
  const fetchCommissionReport = async () => {
    if (!canCreateCommissionReport) {
      showError('You do not have permission to generate commission report');
      return;
    }
  ...
  if (!canViewCommissionReport) {
    return (
      <div className="alert alert-danger m-3" role="alert">
        You do not have permission to view Commission Report.
      </div>
    );
  }
            ...
            <CCardBody>
              <div className="mb-3">
                ...
                {isSubdealer ? (
                  ...
                  <CFormSelect
                    ...
                    value={selectedSubdealer}
                    onChange={handleSubdealerChange}
                    disabled={isLoading || !canViewCommissionReport}
                    className={!selectedSubdealer ? 'border-warning' : ''}
                  >
                    <option value="">{isLoading ? 'Loading subdealers...' : 'Choose a subdealer'}</option>
                    {subdealers.map((subdealer) => (
                ...
                <CCol md={12}>
                  <div className="mb-3">
                    ...
                    <CFormSelect 
                      ...
                      disabled={!canViewCommissionReport}
                    >
                      {years.map((year) => (
                        <option key={year} value={year}>
                          {year}
                      ...
                      <CRow className="g-2">
                        {months.map((month) => (
                          <CCol xs={4} sm={3} key={month.value}>
                            <CButton
                              color={selectedMonth === month.value ? 'primary' : 'outline-secondary'}
                              disabled={month.disabled || !canViewCommissionReport}
                              className="w-100 month-button"
                              onClick={() => handleMonthSelect(month.value)}
                              size="sm"
                            >
              ...
              <div className="d-grid gap-2 mt-4">
                <CButton
                  ...
                  size="lg"
                  onClick={fetchCommissionReport}
                  disabled={!selectedSubdealer || !selectedMonth || loadingReport || !canCreateCommissionReport}
                  className="fw-semibold"
                >
                  {loadingReport ? (
                    <>
                ...
                {!canCreateCommissionReport && selectedSubdealer && selectedMonth && (
                  <div className="alert alert-warning text-center small">
                    You don't have permission to generate commission reports
                  </div>
                )}
              </div>
            </CCardBody>
