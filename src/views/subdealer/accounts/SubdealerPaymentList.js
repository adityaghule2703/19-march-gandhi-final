...
const CommissionPayments = () => {
  ...
  useEffect(() => {
    ...
    const fetchSubdealers = async () => {
      try {
        ...
        // If user is a subdealer, auto-select their subdealer
        if (isSubdealer && userSubdealer) {
          const subdealerData = allSubdealers.find(s => s.id === userSubdealer._id);
          if (subdealerData) {
            setSelectedSubdealer(subdealerData);
          } else {
            // If subdealer not found in the list, create a mock object from user data
            setSelectedSubdealer({
              id: userSubdealer._id,
              name: userSubdealer.name,
              location: userSubdealer.latLong?.address || '',
              type: userSubdealer.type
            });
          }
        }
      } catch (err) {
    ...
    fetchSubdealers();
  }, [canViewPaymentSummary, isSubdealer, userSubdealer]);
  ...
  const handleClearFilter = () => {
    // For subdealers, only clear month and year, keep subdealer selected
    if (isSubdealer && userSubdealer) {
      setSelectedMonth('');
      setSelectedYear('');
    } else {
  ...
  const handleGenerateReceipt = () => {
    if (!selectedSubdealer || !selectedMonth || !selectedYear) {
      setError('Please select month, and year to generate receipt');
      return;
    }
          ...
          <div className="mb-3">
            ...
            {isSubdealer ? (
              <div>
                <CFormSelect 
                  id="subdealerSelect" 
                  value={selectedSubdealer?.id || ''} 
                  disabled
                  className="bg-light"
                >
                  <option value={selectedSubdealer?.id || ''}>
                    {selectedSubdealer?.name || 'Your Subdealer Account'}
                  </option>
                </CFormSelect>
                <div className="text-muted small mt-1">
                  Subdealers can only view their own commission payments
                </div>
              </div>
            ) : (
              <CFormSelect id="subdealerSelect" onChange={handleSubdealerSelect} value={selectedSubdealer?.id || ''}>
